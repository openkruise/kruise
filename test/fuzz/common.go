...
func GenerateNodeSelectorTerm(cf *fuzz.ConsumeFuzzer, term *corev1.NodeSelectorTerm) error {
	...
	if len(term.MatchExpressions) == 0 {
		term.MatchExpressions = make([]corev1.NodeSelectorRequirement, r.Intn(2)+1)
	}
	if len(term.MatchFields) == 0 {
		term.MatchFields = make([]corev1.NodeSelectorRequirement, r.Intn(2)+1)
	}
	...
	generateRequirements := func(requirements *[]corev1.NodeSelectorRequirement) error {
		for i := range *requirements {
			...
			if req.Operator == corev1.NodeSelectorOpIn || req.Operator == corev1.NodeSelectorOpNotIn {
				if len(req.Values) == 0 {
					req.Values = make([]string, r.Intn(2)+1)
				}
...
func GenerateLabelSelector(cf *fuzz.ConsumeFuzzer, selector *metav1.LabelSelector) error {
	...
	if len(selector.MatchExpressions) == 0 {
		selector.MatchExpressions = make([]metav1.LabelSelectorRequirement, r.Intn(2)+1)
	}
	if len(selector.MatchLabels) == 0 {
		selector.MatchLabels = make(map[string]string, r.Intn(2)+1)
	}
...
func randomLabelPart(canBeEmpty bool) string {
	...
	partLen := r.Intn(64) // len is [0, 63]
	if !canBeEmpty {
		partLen = r.Intn(63) + 1 // len is [1, 63]
	}
