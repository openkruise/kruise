...
var _ = ginkgo.Describe("AdvancedCronJob v1beta1", ginkgo.Label("AdvancedCronJob", "cronjob", "workload", "v1beta1"), func() {
	f := v1beta1.NewDefaultFramework("advancedcronjobs-v1beta1")
	var ns string
	var c clientset.Interface
	var kc kruiseclientset.Interface
	var tester *v1beta1.AdvancedCronJobTester
	var randStr string
	...
	ginkgo.BeforeEach(func() {
		...
		ns = f.Namespace.Name
		tester = v1beta1.NewAdvancedCronJobTester(c, kc, ns)
	...
	ginkgo.Context("Basic AdvancedCronJob v1beta1 functionality [AdvancedCronJobBasic]", func() {
		ginkgo.It("should create and schedule AdvancedCronJob v1beta1 with Job template", func() {
			acjName := "acj-job-v1beta1-" + randStr
			// Use a schedule that runs every minute for testing
			schedule := "*/1 * * * *"

			ginkgo.By("Creating AdvancedCronJob v1beta1 with Job template")
		...
		ginkgo.It("should create and schedule AdvancedCronJob v1beta1 with BroadcastJob template", func() {
			acjName := "acj-broadcast-v1beta1-" + randStr
			// Use a schedule that runs every minute for testing
			schedule := "*/1 * * * *"

			ginkgo.By("Creating AdvancedCronJob v1beta1 with BroadcastJob template")
	...
	ginkgo.Context("AdvancedCronJob v1beta1 pause functionality [AdvancedCronJobPause]", func() {
		ginkgo.It("should pause and resume AdvancedCronJob v1beta1", func() {
			acjName := "acj-pause-v1beta1-" + randStr
			schedule := "*/1 * * * *"

			ginkgo.By("Creating AdvancedCronJob v1beta1")
			acj := tester.CreateTestAdvancedCronJobWithJobTemplate(acjName, schedule)
			gomega.Expect(acj).NotTo(gomega.BeNil())
			...
			ginkgo.By("Pausing AdvancedCronJob v1beta1")
			...
			ginkgo.By("Resuming AdvancedCronJob v1beta1")
