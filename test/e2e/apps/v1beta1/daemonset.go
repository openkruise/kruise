...
import (
	...
	daemonsetcontroller "github.com/openkruise/kruise/pkg/controller/daemonset"
	...
	ginkgo.Context("Basic DaemonSet functionality [DaemonSetBasic]", func() {
		...
		*/
		...
		ginkgo.It("should only inplace image if update daemonset image with inplace update strategy", func() {
			...
			oldNodeList := common.GetReadySchedulableNodesOrDie(f.ClientSet)
			...
			newNodeList := common.GetReadySchedulableNodesOrDie(f.ClientSet)
		...
		ginkgo.It("should ignore not ready nodes during rolling update when annotation is set", ginkgo.Serial, func() {
			...
			ds.Annotations[daemonsetcontroller.IgnoreNotReadyNodes] = "true"
