...
var _ = ginkgo.Describe("UnitedDeployment", ginkgo.Label("UnitedDeployment", "workload"), ginkgo.FlakeAttempts(3), func() {
	f := v1beta1.NewDefaultFramework("uniteddeployment")
	var ns string
	var c clientset.Interface
	var kc kruiseclientset.Interface
	var tester *v1beta1.UnitedDeploymentTester

	ginkgo.BeforeEach(func() {
		c = f.ClientSet
		kc = f.KruiseClientSet
		ns = f.Namespace.Name
		tester = v1beta1.NewUnitedDeploymentTester(c, kc, ns)
	})
	...
	ginkgo.It("adaptive united deployment basic", func() {
		...
		udManager.UnitedDeployment.Spec.Topology.ScheduleStrategy = appsv1beta1.UnitedDeploymentScheduleStrategy{
		...
		_, err := f.KruiseClientSet.AppsV1beta1().UnitedDeployments(udManager.Namespace).Create(context.Background(),
