...
var _ = ginkgo.Describe("CloneSet", ginkgo.Label("CloneSet", "workload"), func() {
	...
	ginkgo.Context("CloneSet Scaling", func() {
		...
		ginkgo.It("scales in normal cases", func() {
			...
			gomega.Expect(cs.Spec.UpdateStrategy.RollingUpdate.PodUpdatePolicy).To(gomega.Equal(appsv1beta1.RecreateCloneSetPodUpdateStrategyType))
			gomega.Expect(cs.Spec.UpdateStrategy.RollingUpdate.MaxUnavailable).To(gomega.Equal(func() *intstr.IntOrString { i := intstr.FromString("20%"); return &i }()))
			...
			gomega.Eventually(func() int32 {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.Replicas
			}, 60*time.Second, time.Second).Should(gomega.Equal(int32(3)))
		...
		ginkgo.It("specific delete a Pod, when scalingExcludePreparingDelete is disabled", func() {
			...
			gomega.Eventually(func() *appsv1beta1.CloneSetCondition {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return finalProgressingCondition
			}, 30*time.Second, 3*time.Second).Should(gomega.Equal(&appsv1beta1.CloneSetCondition{
				Type:               appsv1beta1.CloneSetConditionTypeProgressing,
				Status:             v1.ConditionTrue,
				LastUpdateTime:     metav1.Time{},
				LastTransitionTime: metav1.Time{},
		...
		ginkgo.It("specific scale down with lifecycle and then scale up, when scalingExcludePreparingDelete is enabled", func() {
			...
			cs.Spec.ScaleStrategy.ExcludePreparingDelete = true
			cs.Spec.Template.Labels["lifecycle-hook"] = "true"
			cs.Spec.Lifecycle = &appspub.Lifecycle{
				PreDelete: &appspub.LifecycleHook{
					LabelsHandler: map[string]string{"lifecycle-hook": "true"},
			...
			gomega.Eventually(func() int32 {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.Replicas
			}, 60*time.Second, 3*time.Second).Should(gomega.Equal(int32(3)))
	...
	ginkgo.Context("CloneSet Updating", func() {
		...
		ginkgo.It("in-place update images with the same imageID", func() {
			...
			cs, err = tester.CreateCloneSet(cs)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Expect(cs.Spec.UpdateStrategy.RollingUpdate.PodUpdatePolicy).To(gomega.Equal(appsv1beta1.InPlaceIfPossibleCloneSetPodUpdateStrategyType))
			...
			gomega.Eventually(func() int32 {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.Replicas
			}, 60*time.Second, time.Second).Should(gomega.Equal(int32(1)))
		...
		ginkgo.It(`CloneSet partition="99%", replicas=4, make sure one pod is upgraded`, func() {
			...
			gomega.Eventually(func() int32 {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.Replicas
			}, 60*time.Second, time.Second).Should(gomega.Equal(int32(4)))
		...
		ginkgo.It(`CloneSet Update with EnablePVCReuse=false`, func() {
			updateStrategy := appsv1beta1.CloneSetUpdateStrategy{
				Type: appsv1beta1.RollingUpdateCloneSetUpdateStrategyType,
				RollingUpdate: &appsv1beta1.RollingUpdateCloneSetStrategy{
					PodUpdatePolicy: appsv1beta1.RecreateCloneSetPodUpdateStrategyType,
			...
			cs := tester.NewCloneSet("clone-"+randStr, 4, updateStrategy)
			// Explicitly set EnablePVCReuse to true for the first phase of the test
			cs.Spec.ScaleStrategy.EnablePVCReuse = true
			imageConfig := imageutils.GetConfig(imageutils.Nginx)
			imageConfig.SetRegistry("docker.io/library")
			imageConfig.SetVersion("alpine")
			cs.Spec.MinReadySeconds, cs.Spec.ProgressDeadlineSeconds = 10, &tenMinutes
			...
			// update cloneSet again, and EnablePVCReuse=false
			ginkgo.By("Update cloneSet EnablePVCReuse=false")
			err = tester.UpdateCloneSet(cs.Name, func(cs *appsv1beta1.CloneSet) {
				cs.Spec.ScaleStrategy.EnablePVCReuse = false
			})
		...
		ginkgo.It(`CloneSet regard preparing-update pod as update when scaling`, func() {
			...
			tester.UpdateCloneSet(cs.Name, func(cs *appsv1beta1.CloneSet) {
				cs.Spec.UpdateStrategy.RollingUpdate.Partition = nil
				cs.Spec.UpdateStrategy.RollingUpdate.MaxUnavailable = &intstr.IntOrString{Type: intstr.String, StrVal: "100%"}
			})
			gomega.Eventually(func() bool {
				pods, err = tester.ListPodsForCloneSet(cs.Name)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.Context("CloneSet pre-download images", func() {
		...
		ginkgo.It("pre-download for new image", func() {
			...
			cs := tester.NewCloneSet("clone-"+randStr, 5, appsv1beta1.CloneSetUpdateStrategy{
				Type: appsv1beta1.RollingUpdateCloneSetUpdateStrategyType,
				RollingUpdate: &appsv1beta1.RollingUpdateCloneSetStrategy{
					PodUpdatePolicy: appsv1beta1.InPlaceIfPossibleCloneSetPodUpdateStrategyType,
					Partition:       &partition,
			...
			gomega.Eventually(func() int32 {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.Replicas
			}, 60*time.Second, time.Second).Should(gomega.Equal(int32(5)))
			...
			err = tester.UpdateCloneSet(cs.Name, func(cs *appsv1beta1.CloneSet) {
				// For v1beta1, set the spec field instead of annotation
				if cs.Spec.UpdateStrategy.RollingUpdate.InPlaceUpdateStrategy == nil {
					cs.Spec.UpdateStrategy.RollingUpdate.InPlaceUpdateStrategy = &appspub.InPlaceUpdateStrategy{}
				}
				parallelism := intstr.FromInt(2)
				cs.Spec.UpdateStrategy.RollingUpdate.InPlaceUpdateStrategy.ImagePreDownloadParallelism = &parallelism
				cs.Spec.Template.Spec.Containers[0].Image = common.NewNginxImage
			})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			gomega.Eventually(func() int {
				...
				return len(jobs)
			}, 60*time.Second, time.Second).Should(gomega.Equal(1))
	...
	ginkgo.Context("CloneSet updateStrategy type=OnDelete", func() {
		var err error

		ginkgo.It("CloneSet updateStrategy type=OnDelete", func() {
			cs := tester.NewCloneSet("clone-"+randStr, 5, appsv1beta1.CloneSetUpdateStrategy{
				Type: appsv1beta1.OnDeleteCloneSetUpdateStrategyType,
			})
			cs.Spec.MinReadySeconds, cs.Spec.ProgressDeadlineSeconds = 10, &tenMinutes
			cs, err = tester.CreateCloneSet(cs)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			gomega.Eventually(func() int32 {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.UpdatedAvailableReplicas
			}, 30*time.Second, time.Second).Should(gomega.Equal(int32(5)))
			...
			gomega.Eventually(func() *appsv1beta1.CloneSetCondition {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return condition
			}, 120*time.Minute, 3*time.Second).Should(gomega.Equal(tester.NewCloneSetUpdatedCondition()))
			...
			gomega.Eventually(func() int32 {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.UpdatedReplicas
			}, 60*time.Second, time.Second).Should(gomega.Equal(int32(0)))

			pods, err := tester.ListPodsForCloneSet(cs.Name)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Expect(len(pods)).Should(gomega.Equal(5))

			ginkgo.By("Delete two old pods")
			for i := 0; i < 2; i++ {
				err = tester.DeletePod(pods[i].Name)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}
			...
			gomega.Eventually(func() int32 {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.UpdatedAvailableReplicas
			}, 30*time.Second, time.Second).Should(gomega.Equal(int32(2)))

			ginkgo.By("scale down replicas to 3")
			err = tester.UpdateCloneSet(cs.Name, func(cs *appsv1beta1.CloneSet) {
				cs.Spec.Replicas = utilpointer.Int32(3)
			})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Eventually(func() int32 {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.ReadyReplicas
			}, 30*time.Second, time.Second).Should(gomega.Equal(int32(3)))
			gomega.Eventually(func() int32 {
				cs, err = tester.GetCloneSet(cs.Name)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.UpdatedAvailableReplicas
			...
			ginkgo.By("scale down replicas to 5")
			err = tester.UpdateCloneSet(cs.Name, func(cs *appsv1beta1.CloneSet) {
				cs.Spec.Replicas = utilpointer.Int32(5)
			})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Eventually(func() int32 {
				cs, err = tester.GetCloneSet(cs.Name)
			...
			gomega.Eventually(func() int32 {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return cs.Status.UpdatedAvailableReplicas
			}, 30*time.Second, time.Second).Should(gomega.Equal(int32(4)))

			pods, err = tester.ListPodsForCloneSet(cs.Name)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Expect(len(pods)).Should(gomega.Equal(5))

			ginkgo.By("Delete one old pod")
			for _, pod := range pods {
				if pod.Spec.Containers[0].Image != common.NewNginxImage {
					err = tester.DeletePod(pod.Name)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
					break
				}
			}
...
func changeCloneSetAndWaitReady(tester *v1beta1.CloneSetTester, cs *appsv1beta1.CloneSet,
	...
	gomega.Eventually(func() int32 {
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		return cs.Status.Replicas
	}, 60*time.Second, time.Second).Should(gomega.Equal(replica))
	time.Sleep(time.Second * 3)
...
func testUpdateVolumeClaimTemplates(tester *v1beta1.CloneSetTester, randStr string, c clientset.Interface) {
	...
	updateImageOnly := func(cs *appsv1beta1.CloneSet) {
		imageConfig = imageutils.GetConfig(imageutils.Agnhost)
		cs.Spec.Template.Spec.Containers[0].Image = imageConfig.GetE2EImage()
		cs.Spec.UpdateStrategy = appsv1beta1.CloneSetUpdateStrategy{
			Type: appsv1beta1.RollingUpdateCloneSetUpdateStrategyType,
			RollingUpdate: &appsv1beta1.RollingUpdateCloneSetStrategy{
