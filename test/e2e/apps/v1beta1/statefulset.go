...
var _ = ginkgo.Describe("VolumeExpansion v1beta1", ginkgo.Label("VolumeExpansion", "v1beta1"), func() {
	f := v1alpha1.NewDefaultFramework("statefulset-storage")
	var ns string
	...
	ginkgo.Context("Resize PVC", func() {
		...
		var sst *v1beta1.StatefulSetTester

		ginkgo.BeforeEach(func() {
			ginkgo.By("Creating service " + headlessSvcName + " in namespace " + ns)
			headlessService := v1alpha1.CreateServiceSpec(headlessSvcName, "", true, labels)
		...
		ginkgo.AfterEach(func() {
			...
			apps2.Logf("Deleting all statefulset in ns %v", ns)
			v1beta1.DeleteAllStatefulSets(c, kc, ns)
		})
		newImage := apps2.NewNginxImage
		validateExpandVCT := func(vctNumber int, injectSCFn func(ss *appsv1beta1.StatefulSet), updateFn func(ss *appsv1beta1.StatefulSet), expectErr bool) {
			...
			ss = v1beta1.NewStatefulSet(ssName, ns, headlessSvcName, 3, vms, nil, labels)
			injectSCFn(ss)
			...
			if expectErr {
				// error is expected
				if err == nil {
					apps2.Failf("unexpected to update pvc with sc can not expand, but get error %v", err)
				}
				return
			} else {
				apps2.ExpectNoError(err)
		...
		ginkgo.It("should perform rolling updates with specified-deleted with pvc", func() {
			...
			ss = v1beta1.NewStatefulSet(ssName, ns, headlessSvcName, 4, vms, nil, labels)
			injectSC(appsv1beta1.InPlaceIfPossiblePodUpdateStrategyType, ss, appsv1beta1.OnPodRollingUpdateVolumeClaimUpdateStrategyType, canExpandSC)
	...
	ginkgo.Context("Resize PVC with rollback", func() {
		...
		ginkgo.It("partition 2, rollback only image and update to another image", func() {
			...
			var err error
			ss, err = updateStatefulSetWithRetries(ctx, kc, ns, ss.Name, func(set *appsv1beta1.StatefulSet) {
				set.Spec.Template.Spec.Containers[0].Image = apps2.WebserverImage
			})
			apps2.ExpectNoError(err)
...
// GCE Quota requirements: 3 pds, one per stateful pod manifest declared above.
// GCE Api requirements: nodes and master need storage r/w permissions.
var _ = ginkgo.Describe("StatefulSet v1beta1", ginkgo.Label("StatefulSet", "workload", "v1beta1"), func() {
	f := v1alpha1.NewDefaultFramework("statefulset")
	var ns string
	var c clientset.Interface
	var kc kruiseclientset.Interface
	var serverMinorVersion int
	...
	ginkgo.Context("Basic StatefulSet functionality [StatefulSetBasic]", func() {
		...
		ginkgo.BeforeEach(func() {
			...
			ss = v1beta1.NewStatefulSet(ssName, ns, headlessSvcName, 2, statefulPodMounts, podMounts, labels)
		...
		ginkgo.It("should adopt matching orphans and release non-matching pods", func() {
			...
			*(ss.Spec.Replicas) = 1
			...
			gomega.Expect(apps2.WaitForPodCondition(c, pod.Namespace, pod.Name, "adopted", v1beta1.StatefulSetTimeout,
			...
			gomega.Expect(apps2.WaitForPodCondition(c, pod.Namespace, pod.Name, "released", v1beta1.StatefulSetTimeout,
				func(pod *v1.Pod) (bool, error) {
					controllerRef := metav1.GetControllerOf(pod)
					if controllerRef != nil {
						return false, nil
		...
		*/
		ginkgo.It("should perform rolling updates and roll backs of template modifications", func() {
			ginkgo.By("Creating a new StatefulSet")
			ss := v1beta1.NewStatefulSet("ss2", ns, headlessSvcName, 3, nil, nil, labels)
			rollbackTest(c, kc, ns, ss)
		})
		...
		*/
		ginkgo.It("Scaling should happen in predictable order and halt if any stateful pod is unhealthy", func() {
			...
			ss := v1beta1.NewStatefulSet(ssName, ns, headlessSvcName, 1, nil, nil, psLabels)
			sst := v1beta1.NewStatefulSetTester(c, kc)
			sst.SetHTTPProbe(ss)
			ss, err = kc.AppsV1beta1().StatefulSets(ns).Create(context.TODO(), ss, metav1.CreateOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			ctx, cancel := watchtools.ContextWithOptionalTimeout(context.Background(), v1beta1.StatefulSetTimeout)
			defer cancel()
		...
		*/
		ginkgo.It("Should recreate evicted statefulset", func() {
			...
			nodes := apps2.GetReadySchedulableNodesOrDie(f.ClientSet)
			node := nodes.Items[0]
			...
			ss := v1beta1.NewStatefulSet(ssName, f.Namespace.Name, headlessSvcName, 1, nil, nil, labels)
			...
			if err := f.WaitForPodRunning(podName); err != nil {
				apps2.Failf("Pod %v did not start running: %v", podName, err)
			}
			...
			apps2.ExpectNoError(err)
			ctx, cancel := watchtools.ContextWithOptionalTimeout(context.Background(), v1beta1.StatefulPodTimeout)
			defer cancel()
			...
			_, err = watchtools.UntilWithoutRetry(ctx, w, func(event watch.Event) (bool, error) {
				...
				switch event.Type {
				case watch.Deleted:
					apps2.Logf("Observed delete event for stateful pod %v in namespace %v", pod.Name, pod.Namespace)
					if initialStatefulPodUID == "" {
						return false, nil
					}
					return true, nil
				}
				apps2.Logf("Observed stateful pod in namespace: %v, name: %v, uid: %v, status phase: %v. Waiting for statefulset controller to delete.",
					pod.Namespace, pod.Name, pod.UID, pod.Status.Phase)
				initialStatefulPodUID = pod.UID
				return false, nil
			})
			if err != nil {
				apps2.Failf("Pod %v expected to be re-created at least once", statefulPodName)
			}
			...
			gomega.Eventually(func() error {
				...
				return nil
			}, v1beta1.StatefulPodTimeout, 2*time.Second).Should(gomega.BeNil())
		})
		...
		*/
		ginkgo.It("should have a working scale subresource", func() {
			ginkgo.By("Creating statefulset " + ssName + " in namespace " + ns)
			ss := v1beta1.NewStatefulSet(ssName, ns, headlessSvcName, 1, nil, nil, labels)
			sst := v1beta1.NewStatefulSetTester(c, kc)
			sst.SetHTTPProbe(ss)
			ss, err := kc.AppsV1beta1().StatefulSets(ns).Create(context.TODO(), ss, metav1.CreateOptions{})
			apps2.ExpectNoError(err)
			...
			if err != nil {
				apps2.Failf("Failed to get scale subresource: %v", err)
			}
			v1alpha1.ExpectEqual(scale.Spec.Replicas, int32(1))
			v1alpha1.ExpectEqual(scale.Status.Replicas, int32(1))
			...
			if err != nil {
				apps2.Failf("Failed to put scale subresource: %v", err)
			}
			v1alpha1.ExpectEqual(scaleResult.Spec.Replicas, int32(2))
			...
			if err != nil {
				apps2.Failf("Failed to get statefulset resource: %v", err)
			}
			v1alpha1.ExpectEqual(*(ss.Spec.Replicas), int32(2))
		})
			...
			sst.WaitForState(ss, func(set *appsv1beta1.StatefulSet, pl *v1.PodList) (bool, error) {
				...
				for i := range pods.Items {
					if pods.Items[i].Labels[apps.StatefulSetRevisionLabel] != set.Status.UpdateRevision {
						apps2.Logf("Waiting for Pod %s/%s to have revision %s update revision %s",
							pods.Items[i].Namespace,
							pods.Items[i].Name,
	...
	ginkgo.Context("Non-retain StatefulSetPersistentVolumeClaimPolicy [Feature:StatefulSetAutoDeletePVC]", func() {
		...
		ginkgo.It("should delete PVCs with a WhenDeleted policy", func() {
			if v1alpha1.SkipIfNoDefaultStorageClass(c) {
				return
			}
			ginkgo.By("Creating statefulset " + ssName + " in namespace " + ns)
			*(ss.Spec.Replicas) = 3
		...
		ginkgo.It("should delete PVCs with a OnScaledown policy", func() {
			...
			*(ss.Spec.Replicas) = 3
			...
			ss, err = v1beta1.NewStatefulSetTester(c, kc).Scale(ss, 1)
			apps2.ExpectNoError(err)
		...
		ginkgo.It("should delete PVCs after adopting pod (WhenDeleted)", func() {
			...
			*(ss.Spec.Replicas) = 3
			...
			apps2.ExpectNoError(err, "Could not Marshal JSON for patch payload")
			...
			apps2.ExpectNoError(err, "Could not patch payload")
	...
	ginkgo.Context("Automatically recreate PVC for pending pod when PVC is missing", func() {
		...
		ginkgo.BeforeEach(func() {
			statefulPodMounts = []v1.VolumeMount{{Name: "datadir", MountPath: "/data/"}}
			ss = v1beta1.NewStatefulSet(ssName, ns, headlessSvcName, 1, statefulPodMounts, nil, labels)
		})
		...
		ginkgo.It("PVC should be recreated when pod is pending due to missing PVC", func() {
			ctx := context.TODO()
			v1alpha1.SkipIfNoDefaultStorageClass(c)

			readyNode, err := v1alpha1.GetRandomReadySchedulableNode(ctx, c)
			apps2.ExpectNoError(err)
			...
			v1alpha1.WaitForNodeSchedulable(ctx, c, nodeName, 10*time.Second, false)
	...
	ginkgo.Context("Scaling StatefulSetStartOrdinal", func() {
		...
		ginkgo.BeforeEach(func() {
			ss = v1beta1.NewStatefulSet(ssName, ns, headlessSvcName, 2, nil, nil, labels)
		...
		ginkgo.It("Removing .start.ordinal", func() {
			...
			*(ss.Spec.Replicas) = 2
			...
			// since we are replacing 2 pods for 2, we need to ensure we wait
			// for the new ones to show up, not just for any random 2
			apps2.Logf("Confirming 2 replicas, with start ordinal 0")
			waitForStatus(ctx, c, kc, ss)
...
func verifyStatefulSetPVCsExist(c clientset.Interface, ss *appsv1beta1.StatefulSet, claimIds []int) error {
	...
	return wait.PollUntilContextTimeout(context.Background(), v1beta1.StatefulSetPoll, v1beta1.StatefulSetTimeout, true, func(_ context.Context) (bool, error) {
		...
		if err != nil {
			apps2.Logf("WARNING: Failed to list pvcs for verification, retrying: %v", err)
			return false, nil
		}
		for _, claim := range ss.Spec.VolumeClaimTemplates {
			...
			for _, pvc := range pvcList.Items {
				...
				if err != nil {
					apps2.Logf("ERROR: bad pvc name %s (%v)", pvc.Name, err)
					return false, err
				}
			...
			if len(seenPVCs) != len(idSet) {
				apps2.Logf("Found %d of %d PVCs", len(seenPVCs), len(idSet))
				return false, nil // Retry until the PVCs are consistent.
			}
		}
		return true, nil
...
func verifyStatefulSetPVCsExistWithOwnerRefs(c clientset.Interface, kc kruiseclientset.Interface, ss *appsv1beta1.StatefulSet, claimIndices []int, wantSetRef, wantPodRef bool) error {
	...
	setUID := set.GetUID()
	if setUID == "" {
		apps2.Failf("Statefulset %s missing UID", ss.Name)
	}
	return wait.PollUntilContextTimeout(context.Background(), v1beta1.StatefulSetPoll, v1beta1.StatefulSetTimeout, true, func(_ context.Context) (bool, error) {
		pvcList, err := c.CoreV1().PersistentVolumeClaims(ss.Namespace).List(context.TODO(), metav1.ListOptions{LabelSelector: klabels.Everything().String()})
		if err != nil {
		...
		for _, claim := range ss.Spec.VolumeClaimTemplates {
			...
			for _, pvc := range pvcList.Items {
				...
				if _, found := indexSet[int(ordinal)]; !found {
					apps2.Logf("Unexpected, retrying")
					return false, nil // Retry until the PVCs are consistent.
				}
				...
				for _, ref := range pvc.GetOwnerReferences() {
					...
					if ref.Kind == "Pod" {
						...
						if err != nil {
							apps2.Logf("Pod %s not found, retrying (%v)", podName, err)
							return false, nil
						}
						podUID := pod.GetUID()
						if podUID == "" {
							apps2.Failf("Pod %s is missing UID", pod.Name)
						}
						if ref.UID == podUID {
							foundPodRef = true
						}
			...
			if len(seenPVCs) != len(indexSet) {
				apps2.Logf("Only %d PVCs, retrying", len(seenPVCs))
				return false, nil // Retry until the PVCs are consistent.
			}
		}
		return true, nil
...
func expectPodNames(actualPods *v1.PodList, expectedPodNames []string) error {
	v1beta1.SortStatefulPods(actualPods)
	pods := []string{}
...
func updateStatefulSetWithRetries(ctx context.Context, kc kruiseclientset.Interface, namespace, name string, applyUpdate updateStatefulSetFunc) (statefulSet *appsv1beta1.StatefulSet, err error) {
	...
	pollErr := wait.PollUntilContextTimeout(ctx, 10*time.Millisecond, 1*time.Minute, true, func(ctx context.Context) (bool, error) {
		...
		if statefulSet, err = statefulSets.Update(ctx, statefulSet, metav1.UpdateOptions{}); err == nil {
			apps2.Logf("Updating stateful set %s", name)
			return true, nil
		}
		updateErr = err
		return false, nil
	...
	sst.WaitForPVCState(set,
		func(intSet *appsv1beta1.StatefulSet, pvcs *v1.PersistentVolumeClaimList) (bool, error) {
			for _, pvc := range pvcs.Items {
				templateName, err := v1beta1.GetVolumeTemplateName(pvc.Name, set.Name)
				if err != nil {
					continue
				}
				if pvc.Status.Capacity != nil {
					...
					apps2.Logf("template %v, spec %v, status %v",
