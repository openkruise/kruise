...
var _ = SIGDescribe("sidecarset", func() {
	...
	framework.KruiseDescribe("SidecarSet HotUpgrade functionality [SidecarSetHotUpgrade]", func() {
		...
		ginkgo.It("sidecarSet upgrade hot sidecar container image", func() {
			...
			podIn := &pods[0]
			workSidecarContainer := util.GetContainer(sidecarcontrol.GetHotUpgradeContainerInfos(podIn)["nginx-sidecar"], podIn)
			...
			emptySidecarContainer := util.GetContainer(sidecarcontrol.GetEmptyHotUpgradeContainer("nginx-sidecar", podIn), podIn)
		...
		ginkgo.It("sidecarSet upgrade hot sidecar container failed image", func() {
			...
			// pod[0]
			podIn1 := &pods[0]
			workSidecarContainer = util.GetContainer(sidecarcontrol.GetHotUpgradeContainerInfos(podIn1)["nginx-sidecar"], podIn1)
			gomega.Expect(workSidecarContainer.Name).To(gomega.Equal("nginx-sidecar-2"))
			gomega.Expect(workSidecarContainer.Image).To(gomega.Equal("nginx:1.19"))
			emptySidecarContainer = util.GetContainer(sidecarcontrol.GetEmptyHotUpgradeContainer("nginx-sidecar", podIn1), podIn1)
			gomega.Expect(emptySidecarContainer.Name).To(gomega.Equal("nginx-sidecar-1"))
			gomega.Expect(emptySidecarContainer.Image).To(gomega.Equal("busybox:latest"))
			// pod[1]
			podIn2 := &pods[1]
			workSidecarContainer = util.GetContainer(sidecarcontrol.GetHotUpgradeContainerInfos(podIn2)["nginx-sidecar"], podIn2)
			gomega.Expect(workSidecarContainer.Name).To(gomega.Equal("nginx-sidecar-2"))
			gomega.Expect(workSidecarContainer.Image).To(gomega.Equal("nginx:1.19"))
			emptySidecarContainer = util.GetContainer(sidecarcontrol.GetEmptyHotUpgradeContainer("nginx-sidecar", podIn2), podIn2)
			gomega.Expect(emptySidecarContainer.Name).To(gomega.Equal("nginx-sidecar-1"))
			gomega.Expect(emptySidecarContainer.Image).To(gomega.Equal("busybox:latest"))
