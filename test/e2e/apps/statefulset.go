...
import (
	...
	"github.com/kruiseio/kruise/test/e2e/framework"
...
var _ = SIGDescribe("StatefulSet", func() {
	...
	framework.KruiseDescribe("Basic StatefulSet functionality [StatefulSetBasic]", func() {
		...
		BeforeEach(func() {
			...
			localPVUUID = string(uuid.NewUUID())
			By("Creating PVs in /tmp/" + localPVUUID)
			err = framework.CreateStatefulSetPVs(c, localPVUUID, 5)
			Expect(err).NotTo(HaveOccurred())
		})
		...
		AfterEach(func() {
			...
			err := framework.CleanStatefulSetPVs(c)
			Expect(err).NotTo(HaveOccurred())
		})
