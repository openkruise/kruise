	...
	framework.KruiseDescribe("PodProbeMarker functionality", func() {
		...
		ginkgo.It("pod probe marker test1", func() {
			...
			for _, pod := range pods {
				...
				condition := util.GetCondition(pod, "healthy")
				gomega.Expect(condition).NotTo(gomega.BeNil())
				gomega.Expect(string(condition.Status)).To(gomega.Equal(string(appsv1alpha1.ProbeTrue)))
				condition = util.GetCondition(pod, "check")
			...
			for _, pod := range pods {
				...
				condition := util.GetCondition(pod, "healthy")
				gomega.Expect(condition).NotTo(gomega.BeNil())
				gomega.Expect(string(condition.Status)).To(gomega.Equal(string(appsv1alpha1.ProbeFalse)))
				// check probe
				gomega.Expect(pod.Annotations["controller.kubernetes.io/pod-deletion-cost"]).To(gomega.Equal("-10"))
				condition = util.GetCondition(pod, "check")
				gomega.Expect(condition).NotTo(gomega.BeNil())
