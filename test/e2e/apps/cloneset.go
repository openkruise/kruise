...
var _ = SIGDescribe("CloneSet", func() {
	...
	framework.KruiseDescribe("CloneSet Creating", func() {
		...
		framework.ConformanceIt("creates with hostNetwork pod, specified containerPort without specifying hostPort", func() {
			...
			gomega.Eventually(func() *appsv1alpha1.CloneSetCondition {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return condition
			}, 10*time.Second, 3*time.Second).Should(gomega.Equal(tester.NewCloneSetAvailableCondition()))
		})
		...
		framework.ConformanceIt("creates with lifecycle preNormal finalizer", func() {
			...
			cs.Spec.ProgressDeadlineSeconds = ptr.To(int32(10))
			...
			gomega.Eventually(func() *appsv1alpha1.CloneSetCondition {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return condition
			}, 30*time.Second, 3*time.Second).Should(gomega.Equal(tester.NewCloneSetDeadlineExceededCondition(cs.Status.UpdateRevision)))
			...
			gomega.Eventually(func() *appsv1alpha1.CloneSetCondition {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return condition
			}, 10*time.Second, 3*time.Second).Should(gomega.Equal(tester.NewCloneSetDeadlineExceededCondition(cs.Status.UpdateRevision)))
		})
	...
	framework.KruiseDescribe("CloneSet Scaling", func() {
		...
		framework.ConformanceIt("scales with minReadySeconds and scaleStrategy", func() {
			...
			ginkgo.By("Check cloneSet progressing condition with progressing reason")
			gomega.Eventually(func() *appsv1alpha1.CloneSetCondition {
				cs, err = tester.GetCloneSet(cs.Name)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				if cs.Status.AvailableReplicas != replicas {
					condition, err := tester.GetCloneSetProgressingConditionWithoutTime(cs.Name)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
					return condition
				}
				return nil
			}, 10*time.Second, 3*time.Second).Should(gomega.Equal(tester.NewCloneSetUpdatedCondition()))
		...
		framework.ConformanceIt("pods should be ready when paused=true", func() {
			...
			gomega.Eventually(func() *appsv1alpha1.CloneSetCondition {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				return condition
			}, 10*time.Second, 3*time.Second).Should(gomega.Equal(tester.NewCloneSetPausedCondition()))
		})
