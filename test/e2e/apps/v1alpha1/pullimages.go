...
var _ = ginkgo.Describe("PullImage", ginkgo.Label("PullImage", "operation"), ginkgo.Serial, func() {
	f := v1alpha1.NewDefaultFramework("pullimages")
	var ns string
	var c clientset.Interface
	var kc kruiseclientset.Interface
	var testerForNodeImage *v1alpha1.NodeImageTester
	var testerForImagePullJob *v1alpha1.ImagePullJobTester
	var nodes []*v1.Node
	...
	f.AfterEachActions = []func(){
		func() {
			...
			if ginkgo.CurrentSpecReport().Failed() {
				...
				if err != nil {
					common.Logf("[FAILURE_DEBUG] List NodeImages error: %v", err)
				} else {
					common.Logf("[FAILURE_DEBUG] List NodeImages: %v", util.DumpJSON(nodeImageList))
				}
	...
	ginkgo.BeforeEach(func() {
		...
		ns = f.Namespace.Name
		testerForNodeImage = v1alpha1.NewNodeImageTester(c, kc)
		testerForImagePullJob = v1alpha1.NewImagePullJobTester(c, kc)
	...
	ginkgo.Context("ImagePullJob pulling images functionality [ImagePullJob]", func() {
			...
			job.Spec = appsv1alpha1.ImagePullJobSpec{
				...
				ImagePullJobTemplate: appsv1alpha1.ImagePullJobTemplate{
					Selector: &appsv1alpha1.ImagePullJobNodeSelector{LabelSelector: metav1.LabelSelector{MatchExpressions: []metav1.LabelSelectorRequirement{
						{Key: v1alpha1.FakeNodeImageLabelKey, Operator: metav1.LabelSelectorOpDoesNotExist},
					}}},
		...
		ginkgo.It("create two jobs to pull a same image", func() {
			...
			job1.Spec = appsv1alpha1.ImagePullJobSpec{
				Image: common.NewWebserverImage,
				ImagePullJobTemplate: appsv1alpha1.ImagePullJobTemplate{
					Selector: &appsv1alpha1.ImagePullJobNodeSelector{Names: []string{nodes[0].Name}},
					PullPolicy: &appsv1alpha1.PullPolicy{
						TimeoutSeconds: ptr.To(int32(50)),
