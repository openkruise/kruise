...
var _ = ginkgo.Describe("CloneSet", ginkgo.Label("CloneSet", "workload"), func() {
	f := v1alpha1.NewDefaultFramework("clonesets")
	var ns string
	...
	ginkgo.Context("CloneSet Creating", func() {
		...
		ginkgo.It("creates with maxInt32 progressDeadlineSeconds", func() {
			cs := tester.NewCloneSet("pds-"+randStr, 1, appsv1alpha1.CloneSetUpdateStrategy{})
			...
			err = tester.UpdateCloneSet(cs.Name, func(cs *appsv1alpha1.CloneSet) { cs.Spec.ProgressDeadlineSeconds = &tenMinutes })
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.Context("CloneSet Updating", func() {
		...
		ginkgo.It("in-place update two container images with priorities, should not update the next when the previous one failed", func() {
			...
			gomega.Eventually(func() *v1.ContainerStateTerminated {
				...
				pod = pods[0]
				redisContainerStatus := util.GetContainerStatus("redis", pod)
				if redisContainerStatus == nil {
					return nil
				}
				return redisContainerStatus.LastTerminationState.Terminated
			}, 120*time.Second, time.Second).ShouldNot(gomega.BeNil())
		...
		ginkgo.It("in-place update two container image and env from metadata with priorities", func() {
			...
			gomega.Eventually(func() bool {
				...
				return nginxContainerStatus.LastTerminationState.Terminated.FinishedAt.After(redisContainerStatus.State.Running.StartedAt.Time.Add(time.Second * 10))
			}, 120*time.Second, time.Second).Should(gomega.BeTrue(), "nginx finish time should be after redis start time + 10s")
...
func testChangePodQOS(tester *v1alpha1.CloneSetTester, randStr string, c clientset.Interface) {
	cs := tester.NewCloneSet("clone-"+randStr, 1, appsv1alpha1.CloneSetUpdateStrategy{Type: appsv1alpha1.InPlaceIfPossibleCloneSetUpdateStrategyType})
	cs.Spec.Template.Spec.Containers[0].Image = common.NginxImage
	cs.Spec.Template.ObjectMeta.Labels["test-env"] = "foo"
	cs.Spec.Template.Spec.Containers[0].Env = append(cs.Spec.Template.Spec.Containers[0].Env, v1.EnvVar{
...
func changeCloneSetAndWaitReady(tester *v1alpha1.CloneSetTester, cs *appsv1alpha1.CloneSet,
...
func testUpdateVolumeClaimTemplates(tester *v1alpha1.CloneSetTester, randStr string, c clientset.Interface) {
	updateStrategy := appsv1alpha1.CloneSetUpdateStrategy{Type: appsv1alpha1.RecreateCloneSetUpdateStrategyType}
	var replicas int = 4
