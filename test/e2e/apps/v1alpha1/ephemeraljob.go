...
var _ = ginkgo.Describe("EphemeralJob", ginkgo.Label("EphemeralJob", "job", "workload"), func() {
	f := v1alpha1.NewDefaultFramework("ehpemeraljobs")
	var ns string
	var c clientset.Interface
	var kc kruiseclientset.Interface
	var tester *v1alpha1.EphemeralJobTester
	var randStr string
	...
	ginkgo.BeforeEach(func() {
		...
		tester = v1alpha1.NewEphemeralJobTester(c, kc, ns)
	...
	ginkgo.Context("EphemeralJob Creating", func() {
		ginkgo.AfterEach(func() {
			...
			common.Logf("Deleting all EphemeralJob in cluster")
	...
	ginkgo.Context("Create Ephemeral Container", func() {
		ginkgo.AfterEach(func() {
			...
			common.Logf("Deleting all ephemeral container in cluster")
			tester.DeleteEphemeralJobs(ns)
			tester.DeleteDeployments(ns)
		})
		...
		ginkgo.It("test ephemeral container in in-place-update", func() {
			...
			cloneSetTester := v1alpha1.NewCloneSetTester(c, kc, ns)
		...
		ginkgo.It("test ephemeral container in crr", func() {
			...
			{
				restartContainerTester := frameworkv1beta1.NewContainerRecreateTester(c, kc, ns)
				...
				gomega.Eventually(func() string {
					return crr.Labels[appsv1beta1.ContainerRecreateRequestPodUIDKey]
				...
				crr = restartContainerTester.WaitForCRRCompleted(crr.Name, 70*time.Second)
				...
				gomega.Expect(crr.Status.ContainerRecreateStates).Should(gomega.Equal([]appsv1beta1.ContainerRecreateRequestContainerRecreateState{{Name: "nginx", Phase: appsv1beta1.ContainerRecreateRequestSucceeded, IsKilled: true}}))
