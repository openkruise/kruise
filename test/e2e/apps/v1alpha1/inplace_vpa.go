...
var tester *v1alpha1.CloneSetTester
...
var _ = ginkgo.Describe("InplaceVPA", ginkgo.Label("InplaceVPA", "operation"), func() {
	...
	f := v1alpha1.NewDefaultFramework("inplace-vpa")
	var ns string
	...
	ginkgo.BeforeEach(func() {
		...
		ns = f.Namespace.Name
		tester = v1alpha1.NewCloneSetTester(c, kc, ns)
	...
	ginkgo.Context("Basic DaemonSet functionality [DaemonSetBasic]", func() {
		...
		ginkgo.AfterEach(func() {
			...
			apps2.Logf("Deleting DaemonSet %s/%s in cluster", ns, dsName)
		...
		ginkgo.It("should upgrade resources and image one by one on steps if there is pre-update hook", func() {
			...
			ads := tester.NewDaemonSet(dsName, label, apps2.WebserverImage, appsv1alpha1.DaemonSetUpdateStrategy{
			...
			err = wait.PollUntilContextTimeout(context.TODO(), v1alpha1.DaemonSetRetryPeriod, v1alpha1.DaemonSetRetryTimeout, true, func(ctx context.Context) (bool, error) {
			...
			gomega.Eventually(func() int32 {
				...
				apps2.Logf("UpdatedNumber %v Unavailable %v UpdatedAvailable %v", ads.Status.UpdatedNumberScheduled,
...
func checkPodResource(pods []*v1.Pod, last map[string]*v1.ResourceRequirements, ignoreContainer []string) (res bool) {
	defer func() {
		if !res && len(last) == 1 {
			...
			apps2.Logf("pod %v, last resource %v", string(pjson), string(ljson))
		}
	}()
	for _, pod := range pods {
		...
		for _, c := range pod.Spec.Containers {
			...
			if ResourceEqual(&c.Resources, lastResource) {
				apps2.Logf("container %s resource unchanged", c.Name)
				// resource unchanged
				return false
			}
		}
		for _, cs := range pod.Status.ContainerStatuses {
			...
			if !ResourceEqual(spec, cs.Resources) {
				apps2.Logf("container %v spec != status", cname)
...
func rollbackWithUpdateFnTest(c clientset.Interface, kc kruiseclientset.Interface, ns string, ss *appsv1beta1.StatefulSet,
	...
	ss, err = v1beta1.UpdateStatefulSetWithRetries(kc, ns, ss.Name, updateFn)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ss, err = v1beta1.UpdateStatefulSetWithRetries(kc, ns, ss.Name, rollbackFn)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	ss = sst.WaitForStatus(ss)
	currentRevision, updateRevision = ss.Status.CurrentRevision, ss.Status.UpdateRevision
	gomega.Expect(currentRevision).NotTo(gomega.Equal(updateRevision),
...
func getNewestResourcesInfo(po *v1.Pod, a, b, c *string) {
	...
	if a1 != *a || b1 != *b || c1 != *c {
		apps2.Logf("updateSpec %v", a1)
		apps2.Logf("spec %v", b1)
		apps2.Logf("container status %v ", c1)
		a, b, c = &a1, &b1, &c1
	}
}
...
func updateClone(tester *v1alpha1.CloneSetTester, fn func(pod *v1.PodTemplateSpec), cs *appsv1alpha1.CloneSet, injectFailedResource bool) {
	err := tester.UpdateCloneSet(cs.Name, func(cs *appsv1alpha1.CloneSet) {
		if cs.Annotations == nil {
			cs.Annotations = map[string]string{}
		}
...
func waitCloneSetUpdatedAndReady(tester *v1alpha1.CloneSetTester, cs *appsv1alpha1.CloneSet, oldContainerId string, pods []*v1.Pod) {
	var err error
	a, b, c := getResourcesInfo(pods[0])
	ginkgo.By("Wait for all pods updated and ready")
	gomega.Eventually(func() int32 {
		cs, err = tester.GetCloneSet(cs.Name)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		apps2.Logf("Cloneset updatedReplicas %v updatedReady %v updatedAvailableReplicas %v ",
...
func testUpdateResource(tester *v1alpha1.CloneSetTester, fn func(pod *v1.PodTemplateSpec), resizePolicy []v1.ContainerResizePolicy, injectFailedResource bool) {
	var err error
	...
	cs := tester.NewCloneSet("clone-"+randStr, 1, appsv1alpha1.CloneSetUpdateStrategy{Type: appsv1alpha1.InPlaceIfPossibleCloneSetUpdateStrategyType})
	cs.Spec.Template.Spec.Containers[0].ResizePolicy = resizePolicy
	cs.Spec.Template.Spec.Containers[0].Image = apps2.NginxImage
...
func testResizePolicyFailed(resizePolicy []v1.ContainerResizePolicy) {
	...
	ginkgo.It("in-place update image and resource", func() {
		fn := func(pod *v1.PodTemplateSpec) {
			...
			spec.Containers[0].Image = apps2.NewNginxImage
...
func testWithResizePolicy(resizePolicy []v1.ContainerResizePolicy) {
	...
	ginkgo.It("in-place update two container image, resource with priorities successfully", func() {
		...
		updateClone(tester, func(pod *v1.PodTemplateSpec) {
			pod.Spec.Containers[0].Image = apps2.NewNginxImage
	...
	ginkgo.It("in-place update two container image, resource with priorities, should not update the next when the previous one failed", func() {
		...
		gomega.Expect(inPlaceUpdateState.ContainerBatchesRecord[0].Containers).Should(gomega.Equal([]string{"redis"}))
		gomega.Expect(inPlaceUpdateState.NextContainerImages).Should(gomega.Equal(map[string]string{"nginx": apps2.NewNginxImage}))
