...
var _ = ginkgo.Describe("WorkloadSpread", ginkgo.Label("WorkloadSpread", "operation"), func() {
	f := v1alpha1.NewDefaultFramework("workloadspread")
	workloadSpreadName := "test-workload-spread"
	var c clientset.Interface
	var kc kruiseclientset.Interface
	var ns string
	var tester *v1alpha1.WorkloadSpreadTester
	...
	ginkgo.BeforeEach(func() {
		...
		tester = v1alpha1.NewWorkloadSpreadTester(c, kc, f.DynamicClient)
	...
	ginkgo.Context("WorkloadSpread functionality", func() {
		...
		ginkgo.It("deploy in two zone, the type of maxReplicas is Integer", func() {
			...
			ginkgo.By(fmt.Sprintf("update cloneSet(%s/%s) image=%s", cloneSet.Namespace, cloneSet.Name, common.NewWebserverImage))
		...
		ginkgo.It("elastic deploy for deployment, zone-a=2, zone-b=nil", func() {
			...
			ginkgo.By(fmt.Sprintf("update deployment(%s/%s) image=%s", deployment.Namespace, deployment.Name, common.NewWebserverImage))
