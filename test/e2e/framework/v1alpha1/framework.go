...
func (f *Framework) BeforeEach() {
	...
	if f.ClientSet == nil {
		...
		if common.TestContext.KubeAPIContentType != "" {
			config.ContentType = common.TestContext.KubeAPIContentType
		}
	...
	if !f.SkipNamespaceCreation {
		...
		if common.TestContext.VerifyServiceAccount {
			...
			err = common.WaitForDefaultServiceAccountInNamespace(f.ClientSet, namespace.Name)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		} else {
			common.Logf("Skipping waiting for service account")
		}
...
func (f *Framework) AfterEach() {
	...
	defer func() {
		...
		if common.TestContext.DeleteNamespace && (common.TestContext.DeleteNamespaceOnFailure || !ginkgo.CurrentSpecReport().Failed()) {
			for _, ns := range f.namespacesToDelete {
				...
				if err := common.DeleteNS(f.ClientSet, f.DynamicClient, ns.Name, timeout); err != nil {
					if !apierrors.IsNotFound(err) {
						nsDeletionErrors[ns.Name] = err
					} else {
						common.Logf("Namespace %v was already deleted", ns.Name)
					}
				}
			}
		} else {
			if !common.TestContext.DeleteNamespace {
				common.Logf("Found DeleteNamespace=false, skipping namespace deletion!")
			} else {
				common.Logf("Found DeleteNamespaceOnFailure=false and current test failed, skipping namespace deletion!")
			}
		}
		...
		if len(nsDeletionErrors) != 0 {
			...
			common.Failf("%s", strings.Join(messages, ","))
		}
	}()

	// Print events if the test failed.
	if ginkgo.CurrentSpecReport().Failed() && common.TestContext.DumpLogsOnFailure {
		// Pass both unversioned client and versioned clientset, till we have removed all uses of the unversioned client.
		if !f.SkipNamespaceCreation {
			common.DumpAllNamespaceInfo(f.ClientSet, f.Namespace.Name)
		}
	}
	...
	if err := common.AllNodesReady(f.ClientSet, 3*time.Minute); err != nil {
		common.Failf("All nodes should be ready after test, %v", err)
	}
}

// CreateNamespace is used to create namespace
func (f *Framework) CreateNamespace(baseName string, labels map[string]string) (*v1.Namespace, error) {
	createTestingNS := common.TestContext.CreateTestingNS
	if createTestingNS == nil {
		createTestingNS = common.CreateTestingNS
	}
...
func (f *Framework) WaitForPodRunning(podName string) error {
	return common.WaitForPodNameRunningInNamespace(f.ClientSet, podName, f.Namespace.Name)
}
