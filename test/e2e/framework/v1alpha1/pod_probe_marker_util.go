...
func (s *PodProbeMarkerTester) CreateStatefulSet(sts *appsv1beta1.StatefulSet) {
	common.Logf("create sts(%s/%s)", sts.Namespace, sts.Name)
...
func (s *PodProbeMarkerTester) WaitForPodProbeMarkerProcessed(ppm *appsv1alpha1.PodProbeMarker) {
	...
	if pollErr != nil {
		common.Failf("Failed waiting for podprobemarker to be processed: %v", pollErr)
	}
}
...
func (s *PodProbeMarkerTester) WaitForPodProbeMarkerDeleted(ppm *appsv1alpha1.PodProbeMarker) {
	...
	if pollErr != nil {
		common.Failf("Failed waiting for podprobemarker to be deleted: %v", pollErr)
	}
}
