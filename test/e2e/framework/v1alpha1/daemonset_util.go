...
func (t *DaemonSetTester) DeleteDaemonSet(namespace, name string) {
	...
	if err != nil {
		common.Logf("delete daemonset(%s/%s) failed: %s", t.ns, name, err.Error())
		return
	}
}
...
func (t *DaemonSetTester) WaitForDaemonSetDeleted(namespace, name string) {
	...
	if pollErr != nil {
		common.Failf("Failed waiting for daemonset to enter Deleted: %v", pollErr)
	}
}
...
func (t *DaemonSetTester) CheckDaemonReady(dsName string) func() (bool, error) {
	return func() (bool, error) {
		...
		if desired != updated {
			common.Logf("DaemonSet %v is not updated. DesiredScheduled: %d, UpdatedNumberScheduled: %d", dsName, desired, updated)
			return false, nil
		}
		if desired != scheduled && desired != ready {
			common.Logf("DaemonSet %v is not ready. DesiredScheduled: %d, CurrentScheduled: %d, Ready: %d", dsName, desired, scheduled, ready)
			return false, nil
		}
		return true, nil
	}
...
func (t *DaemonSetTester) ClearDaemonSetNodeLabels(c clientset.Interface) error {
	nodeList := common.GetReadySchedulableNodesOrDie(c)
...
func (t *DaemonSetTester) SetDaemonSetNodeLabels(nodeName string, labels map[string]string) (*v1.Node, error) {
	...
	err := wait.PollUntilContextTimeout(context.TODO(), DaemonSetRetryPeriod, DaemonSetRetryTimeout, false, func(ctx context.Context) (bool, error) {
		...
		if se, ok := err.(*apierrors.StatusError); ok && se.ErrStatus.Reason == metav1.StatusReasonConflict {
			common.Logf("failed to update node due to resource version conflict")
			return false, nil
		}
		return false, err
	})
	...
	return func() (bool, error) {
		...
		for _, pod := range newPods.Items {
			for _, status := range pod.Status.ContainerStatuses {
				...
				if image != newImage {
					common.Logf("container image of new pod %s is %s, should be %s", pod.Name, image, newImage)
					return false, nil
				} else {
					common.Logf("Pod %s has new image %s", pod.Name, substr[len(substr)-1])
				}
			}
		}
		return true, nil
...
func (t *DaemonSetTester) CheckPodStayInNode(oldNodeList *v1.NodeList, newNodeList *v1.NodeList) func() (bool, error) {
	return func() (bool, error) {
		if len(oldNodeList.Items) != len(newNodeList.Items) {
			common.Logf("newPods not match old Pods")
			return false, nil
		}
		...
		for _, item := range newNodeList.Items {
			if _, ok := mp[item.Name]; ok {
				...
				common.Logf("mp not contain %s key and mp is %v", item.Name, mp)
				return false, nil
			}
		}
		return true, nil
...
func (t *DaemonSetTester) SchedulableNodes(ds *appsv1alpha1.DaemonSet) []string {
	...
	common.ExpectNoError(err)
	nodeNames := make([]string, 0)
	for _, node := range nodeList.Items {
		if !t.CanScheduleOnNode(node, ds) {
			common.Logf("DaemonSet pods can't tolerate node %s with taints %+v, skip checking this node", node.Name, node.Spec.Taints)
			continue
		}
...
func (t *DaemonSetTester) CheckDaemonPodOnNodes(ds *appsv1alpha1.DaemonSet, nodeNames []string) func() (bool, error) {
	return func() (bool, error) {
		...
		if err != nil {
			common.Logf("could not get the pod list: %v", err)
			return false, nil
		}
		pods := podList.Items
		...
		common.Logf("Number of nodes with available pods: %d", len(nodesToPodCount))

		// Ensure that exactly 1 pod is running on all nodes in nodeNames.
		for _, nodeName := range nodeNames {
			if nodesToPodCount[nodeName] > 1 {
				common.Logf("Node %s is running more than one daemon pod", nodeName)
				return false, nil
			}
		}

		common.Logf("Number of running nodes: %d, number of available pods: %d", len(nodeNames), len(nodesToPodCount))
...
func (t *DaemonSetTester) DaemonPodHasReadinessGate(pods []v1.Pod) bool {
	for _, pod := range pods {
		...
		common.Logf("pod %v has readiness gate", pod.Name)
	}
	return true
}
...
func (t *DaemonSetTester) CheckPodHasNotRecreate(oldPods, newPods []v1.Pod) bool {
	...
	for _, pod := range newPods {
		...
		if !has {
			common.Logf("no old pod found for %v", pod.Name)
			return false
		}
		if oldPod.Name != pod.Name {
			common.Logf("old pod name is not the same %v %v", oldPod.Name, pod.Name)
			return false
		}
	}
