...
func (t *NodeTester) CreateFakeNode(randStr string) (node *v1.Node, err error) {
	...
	go func() {
		...
		for {
			...
			if !noNode {
				...
				if err != nil {
					if !errors.IsNotFound(err) {
						common.Logf("Failed to update status of fake Node %s: %v", name, err)
					}
					noNode = true
				}
			}
			...
			if err != nil {
				common.Logf("Failed to get Pods of fake Node %s: %v", name, err)
				return
			}
...
func (t *NodeTester) ListRealNodesWithFake(tolerations []v1.Toleration) ([]*v1.Node, error) {
	...
	common.Logf("ListRealNodesWithFake starts check tolerations %v with nodes (%v)", tolerations, len(nodeList.Items))
	var nodes []*v1.Node
	for i := range nodeList.Items {
		node := &nodeList.Items[i]
		...
		if !isUntolerated {
			...
			common.Logf("ListRealNodesWithFake check node %s not matched because of %v", node.Name, taint)
		}
	}
	return nodes, nil
}
...
func checkWaitListSchedulableNodes(ctx context.Context, c clientset.Interface) (*v1.NodeList, error) {
	nodes, err := common.WaitListSchedulableNodes(c)
	if err != nil {
...
func IsConditionSetAsExpected(node *v1.Node, conditionType v1.NodeConditionType, wantTrue bool) bool {
	return common.IsNodeConditionSetAsExpectedWithoutSilent(node, conditionType, wantTrue)
}
...
func IsConditionSetAsExpectedSilent(node *v1.Node, conditionType v1.NodeConditionType, wantTrue bool) bool {
	return common.IsNodeConditionSetAsExpectedSilent(node, conditionType, wantTrue)
}
...
func GetReadySchedulableNodes(ctx context.Context, c clientset.Interface) (nodes *v1.NodeList, err error) {
	...
	Filter(nodes, func(node v1.Node) bool {
		return IsNodeSchedulable(&node) && common.IsNodeUntainted(&node)
	})
...
func WaitForNodeSchedulable(ctx context.Context, c clientset.Interface, name string, timeout time.Duration, wantSchedulable bool) bool {
	common.Logf("Waiting up to %v for node %s to be schedulable: %t", timeout, name, wantSchedulable)
	for start := time.Now(); time.Since(start) < timeout; time.Sleep(poll) {
		node, err := c.CoreV1().Nodes().Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			common.Logf("Couldn't get node %s", name)
			continue
		}
	...
	common.Logf("Node %s didn't reach desired schedulable status (%t) within %v", name, wantSchedulable, timeout)
	return false
}
