...
func (t *BroadcastJobTester) WaitForBroadcastJobDesired(job *appsv1beta1.BroadcastJob, expectedDesired int32) {
	pollErr := wait.PollUntilContextTimeout(context.TODO(), time.Second, 30*time.Second, true,
		func(ctx context.Context) (bool, error) {
			inner, err := t.kc.AppsV1beta1().BroadcastJobs(job.Namespace).Get(context.TODO(), job.Name, metav1.GetOptions{})
			if err != nil {
				return false, err
			}
...
func (t *BroadcastJobTester) WaitForBroadcastJobSucceeded(job *appsv1beta1.BroadcastJob, expectedSucceeded int32) {
