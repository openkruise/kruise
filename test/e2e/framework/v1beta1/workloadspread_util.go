		...
		Object: map[string]interface{}{
			"spec": map[string]interface{}{
				"tfReplicaSpecs": map[string]interface{}{
					"PS":     map[string]interface{}{"replicas": ps},
					"MASTER": map[string]interface{}{"replicas": master},
					"Worker": map[string]interface{}{"replicas": worker},
				},
			},
		},
	}
...
// CreateWorkloadSpread creates a v1beta1 WorkloadSpread and waits until it is running.
func (t *WorkloadSpreadTester) CreateWorkloadSpread(workloadSpread *appsv1beta1.WorkloadSpread) *appsv1beta1.WorkloadSpread {
	gomega.Eventually(func(g gomega.Gomega) {
		common.Logf("create WorkloadSpread (%s/%s)", workloadSpread.Namespace, workloadSpread.Name)
		...
		common.Logf("create workloadSpread (%s/%s) success", workloadSpread.Namespace, workloadSpread.Name)
...
// CreateWorkloadSpreadV1alpha1 creates a WorkloadSpread via the v1alpha1 client and returns
// the object read back as v1beta1 — exercising the API-server conversion path.
func (t *WorkloadSpreadTester) CreateWorkloadSpreadV1alpha1(ws *appsv1alpha1.WorkloadSpread) *appsv1beta1.WorkloadSpread {
	common.Logf("create WorkloadSpread via v1alpha1 client (%s/%s)", ws.Namespace, ws.Name)
	_, err := t.KC.AppsV1alpha1().WorkloadSpreads(ws.Namespace).Create(context.TODO(), ws, metav1.CreateOptions{})
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	// Wait for generation to be observed
	var beta *appsv1beta1.WorkloadSpread
	pollErr := wait.PollUntilContextTimeout(context.Background(), time.Second, time.Minute, true,
		func(ctx context.Context) (bool, error) {
			beta, err = t.KC.AppsV1beta1().WorkloadSpreads(ws.Namespace).Get(ctx, ws.Name, metav1.GetOptions{})
			if err != nil {
				return false, err
			}
			return beta.Generation == beta.Status.ObservedGeneration, nil
		})
	if pollErr != nil {
		common.Failf("Failed waiting for v1alpha1-created WorkloadSpread to be observed: %v", pollErr)
	}
	common.Logf("WorkloadSpread created via v1alpha1 is visible as v1beta1 (%s/%s)", ws.Namespace, ws.Name)
	return beta
}

func (t *WorkloadSpreadTester) GetWorkloadSpread(namespace, name string) (*appsv1beta1.WorkloadSpread, error) {
	common.Logf("Get WorkloadSpread (%s/%s)", namespace, name)
...
func (t *WorkloadSpreadTester) GetCloneSet(namespace, name string) (*appsv1alpha1.CloneSet, error) {
	common.Logf("Get CloneSet (%s/%s)", namespace, name)
...
func (t *WorkloadSpreadTester) WaitForWorkloadSpreadRunning(ws *appsv1beta1.WorkloadSpread) {
	...
	if pollErr != nil {
		common.Failf("Failed waiting for workloadSpread to enter running: %v", pollErr)
	}
}

func (t *WorkloadSpreadTester) CreatePriorityClass(pc *schedulingv1.PriorityClass) *schedulingv1.PriorityClass {
	gomega.Eventually(func(g gomega.Gomega) {
		common.Logf("create priorityClass (%s)", pc.Name)
		...
		common.Logf("create priorityClass (%s/%s) success", pc.Namespace, pc.Name)
...
func (t *WorkloadSpreadTester) CreateCloneSet(cloneSet *appsv1alpha1.CloneSet) *appsv1alpha1.CloneSet {
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	common.Logf("create cloneSet (%s/%s) success", cloneSet.Namespace, cloneSet.Name)
...
func (t *WorkloadSpreadTester) CreateStatefulSet(statefulSet *appsv1beta1.StatefulSet) *appsv1beta1.StatefulSet {
	common.Logf("create statefulSet (%s/%s)", statefulSet.Namespace, statefulSet.Name)
	_, err := t.KC.AppsV1beta1().StatefulSets(statefulSet.Namespace).Create(context.TODO(), statefulSet, metav1.CreateOptions{})
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	common.Logf("create statefulSet (%s/%s) success", statefulSet.Namespace, statefulSet.Name)
	sts, _ := t.KC.AppsV1beta1().StatefulSets(statefulSet.Namespace).Get(context.TODO(), statefulSet.Name, metav1.GetOptions{})
	return sts
}

func (t *WorkloadSpreadTester) CreateService(svc *corev1.Service) {
	common.Logf("create Service (%s/%s)", svc.Namespace, svc.Name)
...
func (t *WorkloadSpreadTester) CreateDeployment(deployment *appsv1.Deployment) *appsv1.Deployment {
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	common.Logf("create deployment (%s/%s) success", deployment.Namespace, deployment.Name)
...
func (t *WorkloadSpreadTester) CreateJob(job *batchv1.Job) *batchv1.Job {
	common.Logf("create Job (%s/%s)", job.Namespace, job.Name)
	_, err := t.C.BatchV1().Jobs(job.Namespace).Create(context.TODO(), job, metav1.CreateOptions{})
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	common.Logf("create job (%s/%s) success", job.Namespace, job.Name)
...
func (t *WorkloadSpreadTester) CreateTFJob(obj *unstructured.Unstructured, ps, master, worker int) {
	common.Logf("create TFJob (%s/%s)", obj.GetNamespace(), obj.GetName())
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	common.Logf("creating fake pods: PS %d, MASTER %d, Worker: %d", ps, master, worker)
...
func (t *WorkloadSpreadTester) CreateDaemonSet(ads *appsv1alpha1.DaemonSet) *appsv1alpha1.DaemonSet {
	common.Logf("create DaemonSet (%s/%s)", ads.Namespace, ads.Name)
...
func (t *WorkloadSpreadTester) WaitForCloneSetRunning(cloneSet *appsv1alpha1.CloneSet) {
	...
	common.Logf("wait cloneSet (%s/%s) running success", cloneSet.Namespace, cloneSet.Name)
}
...
func (t *WorkloadSpreadTester) WaitForStatefulSetRunning(statefulSet *appsv1beta1.StatefulSet) {
	...
	if pollErr != nil {
		common.Failf("Failed waiting for statefulSet to enter running: %v", pollErr)
	}
	common.Logf("wait statefulSet (%s/%s) running success", statefulSet.Namespace, statefulSet.Name)
}
...
func (t *WorkloadSpreadTester) WaitJobCompleted(job *batchv1.Job) {
	...
	if pollErr != nil {
		common.Failf("Failed waiting for job to complete: %v", pollErr)
	}
}
...
func (t *WorkloadSpreadTester) UpdateCloneSet(cloneSet *appsv1alpha1.CloneSet) {
	common.Logf("update cloneSet (%s/%s)", cloneSet.Namespace, cloneSet.Name)
...
func (t *WorkloadSpreadTester) UpdateDeployment(deployment *appsv1.Deployment) {
	common.Logf("update deployment (%s/%s)", deployment.Namespace, deployment.Name)
...
func (t *WorkloadSpreadTester) WaiteCloneSetUpdate(cloneSet *appsv1alpha1.CloneSet) {
	...
	common.Logf("wait cloneSet (%s/%s) updated success", cloneSet.Namespace, cloneSet.Name)
}
...
func (t *WorkloadSpreadTester) UpdateWorkloadSpread(workloadSpread *appsv1beta1.WorkloadSpread) {
	common.Logf("update workloadSpread (%s/%s)", workloadSpread.Namespace, workloadSpread.Name)
	...
	if pollErr != nil {
		common.Failf("Failed waiting for workloadSpread to enter ready: %v", pollErr)
	}
}
