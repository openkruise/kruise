/*
Copyright 2015 The Kruise Authors.
...
func (c *PodClient) Update(name string, updateFn func(pod *v1.Pod)) {
	common.ExpectNoError(wait.Poll(time.Millisecond*500, time.Second*30, func() (bool, error) {
		...
		if err == nil {
			common.Logf("Successfully updated pod %q", name)
			return true, nil
		}
		if errors.IsConflict(err) {
			common.Logf("Conflicting update to pod %q, re-get and re-update: %v", name, err)
			return false, nil
		}
