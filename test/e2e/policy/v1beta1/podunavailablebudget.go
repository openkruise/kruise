...
var _ = ginkgo.Describe("PodUnavailableBudget", ginkgo.Label("PodUnavailableBudget", "policy"), func() {
	f := v1beta1.NewDefaultFramework("podunavailablebudget")
	var ns string
	var c clientset.Interface
	var kc kruiseclientset.Interface
	var tester *v1beta1.PodUnavailableBudgetTester
	var sidecarTester *v1beta1.SidecarSetTester

	ginkgo.BeforeEach(func() {
		c = f.ClientSet
		kc = f.KruiseClientSet
		ns = f.Namespace.Name
		tester = v1beta1.NewPodUnavailableBudgetTester(c, kc)
		sidecarTester = v1beta1.NewSidecarSetTester(c, kc)
	})
	...
	ginkgo.Context("podUnavailableBudget functionality [podUnavailableBudget]", func() {
		...
		ginkgo.AfterEach(func() {
			...
			common.Logf("Deleting all PodUnavailableBudgets and Deployments in cluster")
		...
		ginkgo.It("PodUnavailableBudget selector no matched pods", func() {
			...
			expectStatus := &policyv1beta1.PodUnavailableBudgetStatus{
			...
			var err error
			gomega.Eventually(func() *policyv1beta1.PodUnavailableBudgetStatus {
				pub, err = kc.PolicyV1beta1().PodUnavailableBudgets(pub.Namespace).Get(context.TODO(), pub.Name, metav1.GetOptions{})
		...
		ginkgo.It("PodUnavailableBudget selector pods and delete deployment ignore", func() {
			...
			podIn.Annotations[policyv1beta1.PodPubNoProtectionAnnotation] = "true"
		...
		ginkgo.It("PodUnavailableBudget targetReference pods, update failed image and block", func() {
			...
			deployment.Spec.Template.Spec.Containers[0].Image = common.InvalidImage
			_, err = c.AppsV1().Deployments(deployment.Namespace).Update(context.TODO(), deployment, metav1.UpdateOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			time.Sleep(time.Second * 5)
			...
			for _, pod := range pods {
				if pod.Spec.Containers[0].Image == common.InvalidImage || !pod.DeletionTimestamp.IsZero() {
					continue
				}
			...
			deployment.Spec.Template.Spec.Containers[0].Image = common.NewWebserverImage
			_, err = c.AppsV1().Deployments(deployment.Namespace).Update(context.TODO(), deployment, metav1.UpdateOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			for _, pod := range pods {
				...
				gomega.Expect(pod.Spec.Containers[0].Image).To(gomega.Equal(common.NewWebserverImage))
			...
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			podIn1.Spec.Containers[0].Image = common.WebserverImage
			...
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			podIn2.Spec.Containers[0].Image = common.WebserverImage
			_, err = c.CoreV1().Pods(ns).Update(context.TODO(), podIn2, metav1.UpdateOptions{})
			gomega.Expect(err).To(gomega.HaveOccurred())

			// add pub protect operation DELETE
			annotationBody := fmt.Sprintf(`{"metadata":{"annotations":{"%s":"%s"}}}`, policyv1beta1.PubProtectOperationAnnotation, policyv1beta1.PubDeleteOperation)
			_, err = kc.PolicyV1beta1().PodUnavailableBudgets(ns).Patch(context.TODO(), pub.Name, types.MergePatchType, []byte(annotationBody), metav1.PatchOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			time.Sleep(time.Second * 3)
			...
			for _, pod := range pods {
				...
				gomega.Expect(pod.Spec.Containers[0].Image).To(gomega.Equal(common.WebserverImage))
			}
		...
		ginkgo.It("PodUnavailableBudget selector two deployments, deployment.strategy.maxUnavailable=25% and maxSurge=25%, pub.spec.maxUnavailable=25%, and update success image", func() {
			...
			deploymentIn1.Spec.Template.Spec.Containers[0].Image = common.NewWebserverImage
			_, err = c.AppsV1().Deployments(deploymentIn1.Namespace).Update(context.TODO(), deploymentIn1, metav1.UpdateOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			deploymentIn2.Spec.Template.Spec.Containers[0].Image = common.NewWebserverImage
		...
		ginkgo.It("PodUnavailableBudget selector SidecarSet, inject sidecar container, update failed sidecar image, block", func() {
			...
			sidecarSet.Spec.Containers = []appsv1beta1.SidecarContainer{
				{
			...
			sidecarSet.Spec.UpdateStrategy = appsv1beta1.SidecarSetUpdateStrategy{
			...
			sidecarSet, err = kc.AppsV1beta1().SidecarSets().Get(context.TODO(), sidecarSet.Name, metav1.GetOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			sidecarSet.Spec.Containers[0].Image = common.InvalidImage
			...
			sidecarSet, err = kc.AppsV1beta1().SidecarSets().Get(context.TODO(), sidecarSet.Name, metav1.GetOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			sidecarSet.Spec.Containers[0].Image = common.NewNginxImage
			sidecarTester.UpdateSidecarSet(sidecarSet)
		...
		ginkgo.It("PodUnavailableBudget selector cloneSet, strategy.type=recreate, update failed image and block", func() {
			...
			cloneset.Spec.Template.Spec.Containers[0].Image = common.InvalidImage
			_, err = kc.AppsV1beta1().CloneSets(cloneset.Namespace).Update(context.TODO(), cloneset, metav1.UpdateOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			cloneset, _ = kc.AppsV1beta1().CloneSets(cloneset.Namespace).Get(context.TODO(), cloneset.Name, metav1.GetOptions{})
			cloneset.Spec.Template.Spec.Containers[0].Image = common.NewWebserverImage
			cloneset, err = kc.AppsV1beta1().CloneSets(cloneset.Namespace).Update(context.TODO(), cloneset, metav1.UpdateOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			tester.WaitForCloneSetMinReadyAndRunning([]*appsv1beta1.CloneSet{cloneset}, 1)
			...
			newPods := make([]corev1.Pod, 0)
			for _, pod := range pods {
				if !pod.DeletionTimestamp.IsZero() || pod.Spec.Containers[0].Image != common.NewWebserverImage {
					continue
				}
				newPods = append(newPods, *pod)
			}
		...
		ginkgo.It("PodUnavailableBudget selector cloneSet, strategy.type=in-place, update failed image and block", func() {
			...
			// create cloneset
			cloneset := tester.NewBaseCloneSet(ns)
			cloneset.Spec.UpdateStrategy.Type = appsv1beta1.RollingUpdateCloneSetUpdateStrategyType
			cloneset.Spec.UpdateStrategy.RollingUpdate = &appsv1beta1.RollingUpdateCloneSetStrategy{
				PodUpdatePolicy: appsv1beta1.InPlaceOnlyCloneSetPodUpdateStrategyType,
			}
			ginkgo.By(fmt.Sprintf("Creating CloneSet(%s/%s)", cloneset.Namespace, cloneset.Name))
			cloneset = tester.CreateCloneSet(cloneset)
			...
			err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
				cloneset, err = kc.AppsV1beta1().CloneSets(cloneset.Namespace).Get(context.TODO(), cloneset.Name, metav1.GetOptions{})
				if err != nil {
					return err
				}
				cloneset.Spec.Template.Spec.Containers[0].Image = common.NewWebserverImage
		...
		ginkgo.It("PodUnavailableBudget selector two cloneSets, strategy.type=in-place, update success image", func() {
			...
			// update failed image
			ginkgo.By(fmt.Sprintf("update CloneSet(%s/%s) with failed image", cloneset.Namespace, cloneset.Name))
			clonesetIn1.Spec.Template.Spec.Containers[0].Image = common.InvalidImage
			_, err = kc.AppsV1beta1().CloneSets(clonesetIn1.Namespace).Update(context.TODO(), clonesetIn1, metav1.UpdateOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			clonesetIn2.Spec.Template.Spec.Containers[0].Image = common.InvalidImage
			_, err = kc.AppsV1beta1().CloneSets(clonesetIn2.Namespace).Update(context.TODO(), clonesetIn2, metav1.UpdateOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			ginkgo.By(fmt.Sprintf("update CloneSet(%s/%s) success image", cloneset.Namespace, cloneset.Name))
			err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
				clonesetIn1, err = kc.AppsV1beta1().CloneSets(clonesetIn1.Namespace).Get(context.TODO(), clonesetIn1.Name, metav1.GetOptions{})
				if err != nil {
					return err
				}
				clonesetIn1.Spec.Template.Spec.Containers[0].Image = common.NewWebserverImage
				clonesetIn1, err = kc.AppsV1beta1().CloneSets(cloneset.Namespace).Update(context.TODO(), clonesetIn1, metav1.UpdateOptions{})
				return err
			})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			// update success image
			err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
				clonesetIn2, err = kc.AppsV1beta1().CloneSets(clonesetIn2.Namespace).Get(context.TODO(), clonesetIn2.Name, metav1.GetOptions{})
				if err != nil {
					return err
				}
				clonesetIn2.Spec.Template.Spec.Containers[0].Image = common.NewWebserverImage
				clonesetIn2, err = kc.AppsV1beta1().CloneSets(clonesetIn2.Namespace).Update(context.TODO(), clonesetIn2, metav1.UpdateOptions{})
				return err
			})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			tester.WaitForCloneSetMinReadyAndRunning([]*appsv1beta1.CloneSet{clonesetIn1, clonesetIn2}, 7)
		...
		ginkgo.It("PodUnavailableBudget selector cloneSet and sidecarSet, strategy.type=in-place, update success image", func() {
			...
			sidecarTester.UpdateSidecarSet(sidecarSet)
			time.Sleep(time.Second)
			tester.WaitForCloneSetMinReadyAndRunning([]*appsv1beta1.CloneSet{cloneset}, 2)
			exceptSidecarSetStatus := &appsv1beta1.SidecarSetStatus{
				MatchedPods:      10,
				UpdatedPods:      10,
				UpdatedReadyPods: 10,
				ReadyPods:        10,
			...
			newPods := make([]corev1.Pod, 0)
			for _, pod := range pods {
				if pod.DeletionTimestamp.IsZero() && pod.Spec.Containers[1].Image == common.NewWebserverImage && pod.Spec.Containers[0].Image == common.NewNginxImage {
					newPods = append(newPods, *pod)
				}
			}
			gomega.Expect(newPods).To(gomega.HaveLen(10))
...
func setPubStatus(status *policyv1beta1.PodUnavailableBudgetStatus) {
