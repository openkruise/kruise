...
const (
	// CRRPodNameIndex is the lookup name for the most common index function, which is to index by the crr.spec.podName field.
...
func getCurrentCRRContainersRecreateStates(
	...
	for i := range crr.Spec.Containers {
		...
		} else if crr.Spec.Strategy != nil && crr.Spec.Strategy.ForceRecreate && (previousContainerRecreateState == nil || !previousContainerRecreateState.IsKilled) {
...
// getCRRSyncContainerStatuses reads the containerStatusSnapshot from status (v1beta1 typed field).
func getCRRSyncContainerStatuses(crr *appsv1beta1.ContainerRecreateRequest) map[string]*appsv1beta1.ContainerRecreateRequestSyncContainerStatus {
	...
	statuses := make(map[string]*appsv1beta1.ContainerRecreateRequestSyncContainerStatus, len(crr.Status.ContainerStatusSnapshot))
	for i := range crr.Status.ContainerStatusSnapshot {
		s := &crr.Status.ContainerStatusSnapshot[i]
		statuses[s.Name] = s
	}
	return statuses
}
...
func convertCRRToPod(crr *appsv1beta1.ContainerRecreateRequest) *v1.Pod {
	...
	if crr.Spec.Strategy != nil {
		pod.Spec.TerminationGracePeriodSeconds = crr.Spec.Strategy.TerminationGracePeriodSeconds
	}
