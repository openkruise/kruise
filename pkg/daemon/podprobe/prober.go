...
func (pb *prober) probe(p *appsv1alpha1.ContainerProbeSpec, container *runtimeapi.Container, containerID string) (appsv1alpha1.ProbeState, string, error) {
	result, msg, err := pb.runProbeWithRetries(p, container, containerID)
	if bytes.Count([]byte(msg), nil)-1 > maxMessageLength {
		msg = msg[:maxMessageLength]
	}
	if err != nil || (result != probe.Success && result != probe.Warning) {
		return appsv1alpha1.ProbeFalse, msg, err
	}
	return appsv1alpha1.ProbeTrue, msg, nil
}

// runProbeWithRetries tries to probe the container in a finite loop, it returns the last result
// if it never succeeds.
func (pb *prober) runProbeWithRetries(p *appsv1alpha1.ContainerProbeSpec, container *runtimeapi.Container, containerID string) (probe.Result, string, error) {
	var err error
	var result probe.Result
	var output string
	for i := 0; i < maxProbeRetries; i++ {
		result, output, err = pb.runProbe(p, container, containerID)
		if err == nil {
...
func (pb *prober) runProbe(p *appsv1alpha1.ContainerProbeSpec, container *runtimeapi.Container, containerID string) (probe.Result, string, error) {
