...
func TestUpdateNodePodProbeStatus(t *testing.T) {
	cases := []struct {
		...
		getNodePodProbe          func() *appsv1beta1.NodePodProbe
		expectNodePodProbeStatus func() appsv1beta1.NodePodProbeStatus
	}{
		{
			name: "test1, update pod probe status",
			getUpdate: func() Update {
				return Update{Key: probeKey{"", "pod-1", "pod-1-uid", "main", "ppm-1#healthy"}, State: appsv1beta1.ProbeSucceeded}
			},
			getNodePodProbe: func() *appsv1beta1.NodePodProbe {
				demo := demoNodePodProbe.DeepCopy()
				demo.Status = appsv1beta1.NodePodProbeStatus{
					PodProbeStatuses: []appsv1beta1.PodProbeStatus{
						{
							Name: "pod-0",
							UID:  "pod-0-uid",
							ProbeStates: []appsv1beta1.ContainerProbeState{
								{
									Name:  "ppm-1#healthy",
			...
			expectNodePodProbeStatus: func() appsv1beta1.NodePodProbeStatus {
				obj := appsv1beta1.NodePodProbeStatus{
					PodProbeStatuses: []appsv1beta1.PodProbeStatus{
						{
							Name: "pod-0",
							UID:  "pod-0-uid",
		...
		},
		{
			...
			getNodePodProbe: func() *appsv1beta1.NodePodProbe {
				demo := demoNodePodProbe.DeepCopy()
				demo.Spec.PodProbes[1].Probes = append(demo.Spec.PodProbes[1].Probes, appsv1beta1.ContainerProbe{
					Name:          "ppm-1#other",
					ContainerName: "main",
					Probe: appsv1beta1.ContainerProbeSpec{
						Probe: corev1.Probe{
		...
		},
		{
			...
			expectNodePodProbeStatus: func() appsv1beta1.NodePodProbeStatus {
				return appsv1beta1.NodePodProbeStatus{
					PodProbeStatuses: []appsv1beta1.PodProbeStatus{
						{
							Name: "pod-1",
							UID:  "pod-1-uid",
	...
	for _, cs := range cases {
		t.Run(cs.name, func(t *testing.T) {
			...
			c := &Controller{
				...
				nodePodProbeClient:   fakeClient.AppsV1beta1().NodePodProbes(),
				nodeName:             "node-1",
...
func checkNodePodProbeStatusEqual(lister listersalpha1.NodePodProbeLister, expect appsv1beta1.NodePodProbeStatus) bool {
	...
	cases := []struct {
	...
	}{
		{
			...
			setWorkers: func(c *Controller) {
				...
				c.workers[key1] = newWorker(c, key1, &appsv1beta1.ContainerProbeSpec{
					Probe: corev1.Probe{
						ProbeHandler: corev1.ProbeHandler{
							Exec: &corev1.ExecAction{
				...
				c.workers[key2] = newWorker(c, key2, &appsv1beta1.ContainerProbeSpec{
					Probe: corev1.Probe{
						ProbeHandler: corev1.ProbeHandler{
							Exec: &corev1.ExecAction{
			...
			expectWorkers: func(c *Controller) map[probeKey]*worker {
				...
				expect[key] = newWorker(c, key, &appsv1beta1.ContainerProbeSpec{
					Probe: corev1.Probe{
						ProbeHandler: corev1.ProbeHandler{
							Exec: &corev1.ExecAction{
								Command: []string{"/bin/sh", "-c", "/healthy.sh"},
		...
		},
		{
			...
			getNodePodProbe: func() *appsv1beta1.NodePodProbe {
				demo := demoNodePodProbe.DeepCopy()
				demo.Spec.PodProbes[1].Probes = append(demo.Spec.PodProbes[0].Probes, appsv1beta1.ContainerProbe{
					Name:          "ppm-1#check",
					ContainerName: "nginx",
					Probe: appsv1beta1.ContainerProbeSpec{
						Probe: corev1.Probe{
			...
			expectWorkers: func(c *Controller) map[probeKey]*worker {
				...
				expect[key1] = newWorker(c, key1, &appsv1beta1.ContainerProbeSpec{
					Probe: corev1.Probe{
						ProbeHandler: corev1.ProbeHandler{
							Exec: &corev1.ExecAction{
								Command: []string{"/bin/sh", "-c", "/healthy.sh"},
				...
				expect[key2] = newWorker(c, key2, &appsv1beta1.ContainerProbeSpec{
					Probe: corev1.Probe{
						ProbeHandler: corev1.ProbeHandler{
							Exec: &corev1.ExecAction{
								Command: []string{"/bin/sh", "-c", "/check.sh"},
				...
				expect[key3] = newWorker(c, key3, &appsv1beta1.ContainerProbeSpec{
					Probe: corev1.Probe{
						ProbeHandler: corev1.ProbeHandler{
							Exec: &corev1.ExecAction{
								Command: []string{"/bin/sh", "-c", "/healthy.sh"},
