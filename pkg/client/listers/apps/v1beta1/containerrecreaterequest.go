...
type ContainerRecreateRequestLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ContainerRecreateRequest, err error)
...
// List lists all ContainerRecreateRequests in the indexer.
func (s *containerRecreateRequestLister) List(selector labels.Selector) (ret []*v1beta1.ContainerRecreateRequest, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ContainerRecreateRequest))
	})
	return ret, err
}
...
type ContainerRecreateRequestNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ContainerRecreateRequest, error)
...
// List lists all ContainerRecreateRequests in the indexer for a given namespace.
func (s containerRecreateRequestNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.ContainerRecreateRequest, err error) {
...
// Get retrieves the ContainerRecreateRequest from the indexer for a given namespace and name.
func (s containerRecreateRequestNamespaceLister) Get(name string) (*v1beta1.ContainerRecreateRequest, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("containerrecreaterequest"), name)
	}
	return obj.(*v1beta1.ContainerRecreateRequest), nil
}
