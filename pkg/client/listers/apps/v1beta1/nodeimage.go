...
type NodeImageLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.NodeImage, err error)
	// Get retrieves the NodeImage from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.NodeImage, error)
...
// List lists all NodeImages in the indexer.
func (s *nodeImageLister) List(selector labels.Selector) (ret []*v1beta1.NodeImage, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.NodeImage))
	})
	return ret, err
}

// Get retrieves the NodeImage from the index for a given name.
func (s *nodeImageLister) Get(name string) (*v1beta1.NodeImage, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("nodeimage"), name)
	}
	return obj.(*v1beta1.NodeImage), nil
}
