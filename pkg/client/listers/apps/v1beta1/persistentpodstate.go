...
type PersistentPodStateLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.PersistentPodState, err error)
...
// List lists all PersistentPodStates in the indexer.
func (s *persistentPodStateLister) List(selector labels.Selector) (ret []*v1beta1.PersistentPodState, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.PersistentPodState))
	})
	return ret, err
}
...
type PersistentPodStateNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.PersistentPodState, error)
...
// List lists all PersistentPodStates in the indexer for a given namespace.
func (s persistentPodStateNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.PersistentPodState, err error) {
...
// Get retrieves the PersistentPodState from the indexer for a given namespace and name.
func (s persistentPodStateNamespaceLister) Get(name string) (*v1beta1.PersistentPodState, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("persistentpodstate"), name)
	}
	return obj.(*v1beta1.PersistentPodState), nil
}
