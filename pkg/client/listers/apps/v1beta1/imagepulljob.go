...
type ImagePullJobLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ImagePullJob, err error)
...
// List lists all ImagePullJobs in the indexer.
func (s *imagePullJobLister) List(selector labels.Selector) (ret []*v1beta1.ImagePullJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ImagePullJob))
	})
	return ret, err
}
...
type ImagePullJobNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ImagePullJob, error)
...
// List lists all ImagePullJobs in the indexer for a given namespace.
func (s imagePullJobNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.ImagePullJob, err error) {
...
// Get retrieves the ImagePullJob from the indexer for a given namespace and name.
func (s imagePullJobNamespaceLister) Get(name string) (*v1beta1.ImagePullJob, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("imagepulljob"), name)
	}
	return obj.(*v1beta1.ImagePullJob), nil
}
