...
type EphemeralJobLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.EphemeralJob, err error)
...
// List lists all EphemeralJobs in the indexer.
func (s *ephemeralJobLister) List(selector labels.Selector) (ret []*v1beta1.EphemeralJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.EphemeralJob))
	})
	return ret, err
}
...
type EphemeralJobNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.EphemeralJob, error)
...
// List lists all EphemeralJobs in the indexer for a given namespace.
func (s ephemeralJobNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.EphemeralJob, err error) {
...
// Get retrieves the EphemeralJob from the indexer for a given namespace and name.
func (s ephemeralJobNamespaceLister) Get(name string) (*v1beta1.EphemeralJob, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("ephemeraljob"), name)
	}
	return obj.(*v1beta1.EphemeralJob), nil
}
