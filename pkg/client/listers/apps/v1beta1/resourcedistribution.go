...
type ResourceDistributionLister interface {
	...
	List(selector labels.Selector) (ret []*appsv1beta1.ResourceDistribution, err error)
	// Get retrieves the ResourceDistribution from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*appsv1beta1.ResourceDistribution, error)
...
type resourceDistributionLister struct {
	listers.ResourceIndexer[*appsv1beta1.ResourceDistribution]
}
...
func NewResourceDistributionLister(indexer cache.Indexer) ResourceDistributionLister {
	return &resourceDistributionLister{listers.New[*appsv1beta1.ResourceDistribution](indexer, appsv1beta1.Resource("resourcedistribution"))}
}
