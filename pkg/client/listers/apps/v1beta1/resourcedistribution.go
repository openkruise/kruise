...
type ResourceDistributionLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ResourceDistribution, err error)
	// Get retrieves the ResourceDistribution from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ResourceDistribution, error)
...
// List lists all ResourceDistributions in the indexer.
func (s *resourceDistributionLister) List(selector labels.Selector) (ret []*v1beta1.ResourceDistribution, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ResourceDistribution))
	})
	return ret, err
}

// Get retrieves the ResourceDistribution from the index for a given name.
func (s *resourceDistributionLister) Get(name string) (*v1beta1.ResourceDistribution, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("resourcedistribution"), name)
	}
	return obj.(*v1beta1.ResourceDistribution), nil
}
