...
type WorkloadSpreadLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.WorkloadSpread, err error)
...
// List lists all WorkloadSpreads in the indexer.
func (s *workloadSpreadLister) List(selector labels.Selector) (ret []*v1beta1.WorkloadSpread, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.WorkloadSpread))
	})
	return ret, err
}
...
type WorkloadSpreadNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.WorkloadSpread, error)
...
// List lists all WorkloadSpreads in the indexer for a given namespace.
func (s workloadSpreadNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.WorkloadSpread, err error) {
...
// Get retrieves the WorkloadSpread from the indexer for a given namespace and name.
func (s workloadSpreadNamespaceLister) Get(name string) (*v1beta1.WorkloadSpread, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("workloadspread"), name)
	}
	return obj.(*v1beta1.WorkloadSpread), nil
}
