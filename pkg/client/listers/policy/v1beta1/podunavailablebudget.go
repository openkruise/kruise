...
type PodUnavailableBudgetLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.PodUnavailableBudget, err error)
...
// List lists all PodUnavailableBudgets in the indexer.
func (s *podUnavailableBudgetLister) List(selector labels.Selector) (ret []*v1beta1.PodUnavailableBudget, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.PodUnavailableBudget))
	})
	return ret, err
}
...
type PodUnavailableBudgetNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.PodUnavailableBudget, error)
...
// List lists all PodUnavailableBudgets in the indexer for a given namespace.
func (s podUnavailableBudgetNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.PodUnavailableBudget, err error) {
...
// Get retrieves the PodUnavailableBudget from the indexer for a given namespace and name.
func (s podUnavailableBudgetNamespaceLister) Get(name string) (*v1beta1.PodUnavailableBudget, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("podunavailablebudget"), name)
	}
	return obj.(*v1beta1.PodUnavailableBudget), nil
}
