...
func (r *ReconcilePodUnavailableBudget) patchRelatedPubAnnotationInPod(pub *policyv1beta1.PodUnavailableBudget, pods []*corev1.Pod) error {
	...
	for i := range pods {
		if pubcontrol.GetPodRelatedPubName(pods[i]) == "" {
	...
	for _, pod := range updatedPods {
		body := fmt.Sprintf(`{"metadata":{"annotations":{"%s":"%s","%s":"%s"}}}`,
			pubcontrol.PodRelatedPubAnnotation, pub.Name,
			pubcontrol.DeprecatedPodRelatedPubAnnotation, pub.Name)
