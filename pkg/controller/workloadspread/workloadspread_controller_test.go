...
func TestSubsetPodDeletionCost(t *testing.T) {
	cases := []struct {
			...
			expectPods: func() []*corev1.Pod {
				...
				for i := range pods {
					...
					pods[i].Annotations = map[string]string{
						PodDeletionCostAnnotation: PodDeletionCostPositive,
					}
					pods[i].Name = fmt.Sprintf("test-pods-%d", i)
				}
				return pods
		...
		},
		{
			...
			expectPods: func() []*corev1.Pod {
				...
				for i := range pods {
					pods[i] = podDemo.DeepCopy()
					pods[i].Annotations = map[string]string{
						PodDeletionCostAnnotation: PodDeletionCostNegative,
					}
					pods[i].Name = fmt.Sprintf("test-pods-%d", i)
				}
		...
		{
			name: "maxReplicas is nil, pods number is 3, original annotation not set",
			getPods: func() []*corev1.Pod {
				pods := make([]*corev1.Pod, 3)
				for i := range pods {
					pods[i] = podDemo.DeepCopy()
	...
	for _, cs := range cases {
		t.Run(cs.name, func(t *testing.T) {
			...
			err := r.syncSubsetPodDeletionCost(workloadSpread, &workloadSpread.Spec.Subsets[0], cs.getPods(), 5)
			if err != nil {
	...
	cases := []struct {
		...
		},
		{
			...
			expectPods: func() []*corev1.Pod {
				...
				pods[0].Annotations[PodDeletionCostAnnotation] = PodDeletionCostPositive
				pods[1].Annotations[PodDeletionCostAnnotation] = PodDeletionCostPositive
				return pods
			},
			expectWorkloadSpread: func() *appsv1alpha1.WorkloadSpread {
				workloadSpread := workloadSpreadDemo.DeepCopy()
		...
		},
		{
			...
			expectPods: func() []*corev1.Pod {
				...
				pods[0].Annotations[PodDeletionCostAnnotation] = PodDeletionCostNegative
				pods[1].Annotations[PodDeletionCostAnnotation] = PodDeletionCostNegative
				return pods
			},
			expectWorkloadSpread: func() *appsv1alpha1.WorkloadSpread {
				workloadSpread := workloadSpreadDemo.DeepCopy()
