...
func (r *ReconcileEnhancedLivenessProbe2NodeProbe) delNodeProbeConfig(pod *v1.Pod) error {
	return r.retryOnConflictDelNodeProbeConfig(pod)
}

func (r *ReconcileEnhancedLivenessProbe2NodeProbe) addNodeProbeConfig(pod *v1.Pod) error {
	plivenessProbeConfig, err := parseEnhancedLivenessProbeConfig(pod)
	if err != nil {
		return err
	}
	if r.GetPodNodeName(pod) == "" {
		klog.Warningf("No found pod node name, pod: %v/%v", pod.Namespace, pod.Name)
		return nil
	}

	npp := appsv1alpha1.NodePodProbe{}
	err = r.Get(context.TODO(), types.NamespacedName{Name: r.GetPodNodeName(pod)}, &npp)
	if err == nil {
...
func (r *ReconcileEnhancedLivenessProbe2NodeProbe) retryOnConflictDelNodeProbeConfig(pod *v1.Pod) error {
	nppClone := &appsv1alpha1.NodePodProbe{}
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		if err := r.Client.Get(context.TODO(), types.NamespacedName{Name: r.GetPodNodeName(pod)}, nppClone); err != nil {
			if errors.IsNotFound(err) {
				klog.Warningf("No found getting updated npp %s from client", r.GetPodNodeName(pod))
				return nil
			}
			klog.Errorf("Error getting updated npp %s from client", r.GetPodNodeName(pod))
			return err
		}
	...
	if err != nil {
		klog.Errorf("NodePodProbe update NodePodProbe(%s) failed:%s", r.GetPodNodeName(pod), err.Error())
		return err
	}
	return nil
}

func (r *ReconcileEnhancedLivenessProbe2NodeProbe) retryOnConflictAddNodeProbeConfig(pod *v1.Pod,
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if !isHit {
			...
			newPodProbe := generatePodProbeByContainersProbeConfig(pod, containersLivenessProbeConfig)
...
func (r *ReconcileEnhancedLivenessProbe2NodeProbe) generateNewNodePodProbe(pod *v1.Pod) (appsv1alpha1.NodePodProbe, error) {
	...
	npp_tmp := appsv1alpha1.NodePodProbe{
		ObjectMeta: metav1.ObjectMeta{
			Name: r.GetPodNodeName(pod),
		},
	}
	npp_tmp.Spec = appsv1alpha1.NodePodProbeSpec{}
	npp_tmp.Spec.PodProbes = []appsv1alpha1.PodProbe{}
	newPodProbe := generatePodProbeByContainersProbeConfig(pod, podLivenessProbeConfig)
	if len(newPodProbe.Probes) != 0 {
...
func generatePodProbeByContainersProbeConfig(pod *v1.Pod, containersLivenessProbeConfig []livenessprobeUtils.ContainerLivenessProbe) appsv1alpha1.PodProbe {
	...
	newPodContainerProbes := generatePodContainersProbe(pod, containersLivenessProbeConfig)
	newPodProbe.Probes = newPodContainerProbes
	return newPodProbe
}
