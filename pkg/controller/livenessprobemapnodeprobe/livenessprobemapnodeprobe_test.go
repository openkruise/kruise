...
func TestAddOrRemoveNodePodProbeConfig(t *testing.T) {
	...
	for _, tc := range testCase {
		t.Run(tc.name, func(t *testing.T) {
			...
			r := ReconcileEnhancedLivenessProbe2NodeProbe{Client: fakeClient}
			...
			err = r.Get(context.TODO(), types.NamespacedName{Name: r.GetPodNodeName(tc.pod)}, &getNodeProbe)
			if err != nil {
...
func TestSyncPodContainersLivenessProbe(t *testing.T) {
	...
	for _, tc := range testCase {
		t.Run(tc.name, func(t *testing.T) {
			...
			err = fakeClient.Get(context.TODO(), types.NamespacedName{Name: recon.GetPodNodeName(tc.pod)}, &getPodNodeProbe)
			if err != nil {
				t.Errorf("Failed to get pod node probe: %s, err: %v", recon.GetPodNodeName(tc.pod), err)
			}
