...
func FilterOutScheduledFailedPods(notUpdatedPods, updatedPods []*v1.Pod) (newNotUpdated, newUpdated, scheduledFailed []*v1.Pod) {
	for _, p := range notUpdatedPods {
		if isPodScheduledFailed(p) {
			scheduledFailed = append(scheduledFailed, p)
		} else {
			newNotUpdated = append(newNotUpdated, p)
		}
	}
	for _, p := range updatedPods {
		if isPodScheduledFailed(p) {
			...
			newUpdated = append(newUpdated, p)
		}
	}
	return
}

func isPodScheduledFailed(pod *v1.Pod) bool {
	index, cond := podutil.GetPodCondition(&pod.Status, v1.PodScheduled)
	if index == -1 {
		return false
	}
