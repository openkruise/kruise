...
func (r *ReconcileAdvancedCronJob) reconcileImageListPullJob(ctx context.Context, req ctrl.Request, advancedCronJob appsv1beta1.AdvancedCronJob) (ctrl.Result, error) {
	advancedCronJob.Status.Type = appsv1beta1.ImageListPullJobTemplate

	childJobs := &appsv1beta1.ImageListPullJobList{}
	...
	var activeJobs []*appsv1beta1.ImageListPullJob
	var successfulJobs []*appsv1beta1.ImageListPullJob
	var failedJobs []*appsv1beta1.ImageListPullJob

	// helper function to check if an ImageListPullJob is finished
	isImageListPullJobFinished := func(job *appsv1beta1.ImageListPullJob) (bool, appsv1beta1.JobConditionType) {
		if job.Status.CompletionTime != nil {
			if job.Status.Succeeded == job.Status.Desired {
				return true, appsv1beta1.JobComplete
			} else {
				return true, appsv1beta1.JobFailed
			}
		}
	...
	getScheduledTimeForImageListPullJob := func(job *appsv1beta1.ImageListPullJob) (*time.Time, error) {
	...
	for _, activeJob := range activeJobs {
		...
		if err != nil {
			klog.ErrorS(err, "Unable to make reference to active ImageListPullJob", "imageListPullJob", klog.KObj(activeJob), "advancedCronJob", req)
			continue
		}
		advancedCronJob.Status.Active = append(advancedCronJob.Status.Active, *jobRef)
	}

	klog.V(1).InfoS("AdvancedCronJob ImageListPullJob count", "activeJobCount", len(activeJobs), "successfulJobCount", len(successfulJobs), "failedJobCount", len(failedJobs), "advancedCronJob", req)
	...
	*/
	...
	if advancedCronJob.Spec.ConcurrencyPolicy == appsv1beta1.ForbidConcurrent && len(activeJobs) > 0 {
		klog.V(1).InfoS("Concurrency policy blocks concurrent runs, skipping", "activeImageListPulljobs", len(activeJobs), "advancedCronJob", req)
		return scheduledResult, nil
	}
	...
	if advancedCronJob.Spec.ConcurrencyPolicy == appsv1beta1.ReplaceConcurrent {
		for _, activeJob := range activeJobs {
			...
			if err := r.Delete(ctx, activeJob, client.PropagationPolicy(metav1.DeletePropagationBackground)); client.IgnoreNotFound(err) != nil {
				klog.ErrorS(err, "Unable to delete active ImageListPullJob", "job", klog.KObj(activeJob), "advancedCronJob", req)
				return ctrl.Result{}, err
			}
		}
	}
	...
	*/
	constructImageListPullJobForCronJob := func(advancedCronJob *appsv1beta1.AdvancedCronJob, scheduledTime time.Time) (*appsv1beta1.ImageListPullJob, error) {
	...
	if err != nil {
		klog.ErrorS(err, "Unable to construct ImageListPullJob from template", "advancedCronJob", req)
