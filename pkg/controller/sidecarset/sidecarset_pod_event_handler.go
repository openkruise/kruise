...
func isPodConsistentChanged(oldPod, newPod *corev1.Pod, sidecarSet *appsv1alpha1.SidecarSet) (bool, time.Duration) {
	...
	var enqueueDelayTime time.Duration
	oldConsistent := control.IsPodUpdatedConsistent(oldPod, nil)
	newConsistent := control.IsPodUpdatedConsistent(newPod, nil)
	if oldConsistent != newConsistent {
		klog.V(3).Infof("pod(%s.%s) sidecarSet consistent changed(from %v to %v), and reconcile sidecarSet",
