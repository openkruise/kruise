	...
	podHotUpgrade = &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Annotations: map[string]string{
				...
				//sidecar version
				sidecarcontrol.GetSidecarContainerVersionAnnotationKey("test-sidecar-1"): "1",
				sidecarcontrol.GetSidecarContainerVersionAnnotationKey("test-sidecar-2"): "1",
					...
					Env: []corev1.EnvVar{
						{
							...
							ValueFrom: &corev1.EnvVarSource{
								FieldRef: &corev1.ObjectFieldSelector{
									FieldPath: fmt.Sprintf("metadata.annotations['%s']", sidecarcontrol.GetSidecarContainerVersionAnnotationKey("test-sidecar-1")),
								},
							},
						},
					},
					...
					Env: []corev1.EnvVar{
						{
							...
							ValueFrom: &corev1.EnvVarSource{
								FieldRef: &corev1.ObjectFieldSelector{
									FieldPath: fmt.Sprintf("metadata.annotations['%s']", sidecarcontrol.GetSidecarContainerVersionAnnotationKey("test-sidecar-2")),
								},
							},
						},
					},
...
func testUpdateHotUpgradeSidecar(t *testing.T, hotUpgradeEmptyImage string, sidecarSetInput *appsv1alpha1.SidecarSet, handlers map[string]HandlePod) {
	...
	for _, cs := range cases {
		t.Run(cs.name, func(t *testing.T) {
			...
			fakeClient := fake.NewFakeClientWithScheme(scheme, sidecarset, pod)
