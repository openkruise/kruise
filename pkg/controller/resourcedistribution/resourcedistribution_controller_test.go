...
func TestDoReconcile(t *testing.T) {
	...
	resource := &corev1.Secret{}
	if err := reconcileHandler.Client.Get(context.TODO(), types.NamespacedName{Namespace: "ns-4", Name: "test-secret-1"}, resource); err == nil || !errors.IsNotFound(err) {
