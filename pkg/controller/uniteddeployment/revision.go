...
func (r *ReconcileUnitedDeployment) controlledHistories(cd *appsalphav1.UnitedDeployment) ([]*apps.ControllerRevision, error) {
	...
	klog.V(1).Infof("List controller revision of UnitedDeployment %s/%s: count %d\n", cd.Namespace, cd.Name, len(histories.Items))

	getOwner := func() (runtime.Object, error) {
		instance := &appsalphav1.UnitedDeployment{}
		err = r.Client.Get(context.TODO(), client.ObjectKey{Namespace: cd.Namespace, Name: cd.Name}, instance)
		return instance, err
	}

	updateOwnee := func(ownee runtime.Object) (err error) {
		rvs := ownee.(*apps.ControllerRevision)
		return r.Client.Update(context.TODO(), rvs)
	}

	// Use ControllerRefManager to adopt/orphan as needed.
	cm, err := akscontrollerutil.NewRefManager(getOwner, updateOwnee, cd.Spec.Selector, cd, r.scheme)
...
func (r *ReconcileUnitedDeployment) newRevision(cd *appsalphav1.UnitedDeployment, revision int64, collisionCount *int32) (*apps.ControllerRevision, error) {
	patch, err := getUnitedDeploymentPatch(cd)
	if err != nil {
		return nil, err
	}

	gvk, err := apiutil.GVKForObject(cd, r.scheme)
	if err != nil {
		return nil, err
	}

	cr, err := NewControllerRevision(cd,
		gvk,
		cd.Spec.Template.StatefulSetTemplate.Labels,
...
func getUnitedDeploymentPatch(ds *appsalphav1.UnitedDeployment) ([]byte, error) {
