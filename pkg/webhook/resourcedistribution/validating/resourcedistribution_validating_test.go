...
func TestResourceDistributionUpdateValidation(t *testing.T) {
	...
	newRD.Spec.Targets.IncludedNamespaces = append(newRD.Spec.Targets.IncludedNamespaces, appsv1alpha1.ResourceDistributionNamespace{Name: "ns-4"})
...
func TestGetTargetNamespaces(t *testing.T) {
	...
	targetNamespaces, errs := GetTargetNamespaces(handler.Client, &rd.Spec.Targets)
	if len(errs) != 0 {
		t.Fatalf("unexpected error, err %v", errs)

	}
	if len(targetNamespaces) != 2 {
		t.Fatalf("the number of expected target namespace is %d, but got %d", 2, len(targetNamespaces))
	}
}
