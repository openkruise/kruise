...
func testInjectHotUpgradeSidecar(t *testing.T, sidecarSetIn *appsv1alpha1.SidecarSet) {
	...
	if sidecarcontrol.GetHotUpgradeContainerInfos(podOut)["dns-f"] != "dns-f-1" {
	...
	if podOut.Annotations[sidecarcontrol.GetSidecarContainerVersionAnnotationKey("dns-f-1")] != "1" {
		t.Fatalf("pod annotations dns-f-1 version=%s", podOut.Annotations[sidecarcontrol.GetSidecarContainerVersionAnnotationKey("dns-f-1")])
	}
	if podOut.Annotations[sidecarcontrol.GetSidecarContainerVersionAnnotationKey("dns-f-2")] != "0" {
		t.Fatalf("pod annotations dns-f-2 version=%s", podOut.Annotations[sidecarcontrol.GetSidecarContainerVersionAnnotationKey("dns-f-2")])
	}
}
