...
func TestMergeNamespaceSelectors(t *testing.T) {
	tests := []struct {
		...
		{
			name: "duplicate expressions should be deduplicated",
			selector1: &metav1.LabelSelector{
...
// doesn't mutate the configurations in a way that causes update loops. This is a regression test
// for a bug where normalizing nil MatchLabels to empty map caused infinite reconciliation loops.
func TestAreSemanticEqualMutatingWebhooks_NormalizationDoesNotMutate(t *testing.T) {
	...
	// This ensures normalization didn't mutate them in a way that would cause update loops
	if !reflect.DeepEqual(config, configCopy) {
		...
		// Show what changed
		if config.Webhooks[0].NamespaceSelector.MatchLabels == nil &&
			configCopy.Webhooks[0].NamespaceSelector.MatchLabels != nil {
			t.Error("MatchLabels was mutated from nil to empty map - this causes update loops!")
		}
	}
}
