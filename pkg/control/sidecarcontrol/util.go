...
func PodMatchedSidecarSet(c client.Client, pod *corev1.Pod, sidecarSet *appsv1beta1.SidecarSet) (bool, error) {
	...
	if sidecarSet.Spec.SpecificNamespace != nil {
		if sidecarSet.Spec.SpecificNamespace.Namespace != "" && sidecarSet.Spec.SpecificNamespace.Namespace != podNamespace {
			return false, nil
		}
		if sidecarSet.Spec.SpecificNamespace.NamespaceSelector != nil &&
			!IsSelectorNamespace(c, podNamespace, sidecarSet.Spec.SpecificNamespace.NamespaceSelector) {
			return false, nil
		}
	}
...
func FetchSidecarSetMatchedNamespace(c client.Client, sidecarSet *appsv1beta1.SidecarSet) (sets.String, error) {
	ns := sets.NewString()
	// If SpecificNamespace is nil, match pods in all namespaces
	if sidecarSet.Spec.SpecificNamespace == nil {
		nsList := &corev1.NamespaceList{}
		if err := c.List(context.TODO(), nsList, &client.ListOptions{}, utilclient.DisableDeepCopy); err != nil {
			return nil, err
		}
	...
	// If Namespace is not empty, sidecarSet will only match the pods in the namespaces
	if sidecarSet.Spec.SpecificNamespace.Namespace != "" {
		return ns.Insert(sidecarSet.Spec.SpecificNamespace.Namespace), nil
	}
	// get more faster selector
	selector, err := util.ValidatedLabelSelectorAsSelector(sidecarSet.Spec.SpecificNamespace.NamespaceSelector)
	if err != nil {
		return nil, err
	}
	nsList := &corev1.NamespaceList{}
