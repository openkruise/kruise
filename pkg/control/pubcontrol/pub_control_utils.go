...
func checkAndDecrement(pod *corev1.Pod, pub *policyv1alpha1.PodUnavailableBudget, operation policyv1alpha1.PubOperation) error {
	if isPodGroupRecordedInPub(pod, pub) {
		klog.V(3).InfoS("Pod's group was already recorded in pub", "pod", klog.KObj(pod), "pub", klog.KObj(pub))
		return nil
	}
	...
	if groupName := GetPodGroupName(pod, pub); groupName != "" {
		...
		klog.V(3).InfoS("PodGroup was recorded in pub unavailablePodGroups", "podGroupName", groupName, "pub", klog.KObj(pub))
	}
	return nil
}
...
func isPodGroupRecordedInPub(pod *corev1.Pod, pub *policyv1alpha1.PodUnavailableBudget) bool {
	...
	groupName := GetPodGroupName(pod, pub)
	_, exist := pub.Status.UnavailablePodGroups[groupName]
	return exist
}
...
func GetPodGroupName(pod *corev1.Pod, pub *policyv1alpha1.PodUnavailableBudget) string {
	...
	if groupName == "" {
		// Pod missing the group label falls back to using pod name as group name,
		// consistent with the grouping logic in groupPubPods.
		return pod.Name
	}
	return groupName
}
