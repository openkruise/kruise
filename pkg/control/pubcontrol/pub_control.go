...
func groupPubPods(pub *policyv1alpha1.PodUnavailableBudget, pods []*corev1.Pod) map[string][]*corev1.Pod {
	...
	for _, pod := range pods {
		...
		if groupName == "" {
			// Pod missing the group label falls back to legacy per-Pod semantics
			klog.V(3).InfoS("Pod missing group label key, falling back to per-pod semantics",
				"pod", klog.KObj(pod), "groupLabelKey", pub.Spec.PodGroupPolicy.GroupLabelKey)
			groupedPods[pod.Name] = []*corev1.Pod{pod}
			continue
		}
