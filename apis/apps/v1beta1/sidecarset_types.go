...
// SpecificNamespace defines namespace selection for SidecarSet
type SpecificNamespace struct {
...
type SidecarSetSpec struct {
	...
	// SpecificNamespace defines which namespaces the sidecarSet will match pods in.
	// If not specified, match pods in all namespaces(in cluster)
	SpecificNamespace *SpecificNamespace `json:"specificNamespace,omitempty"`
	...
	// CustomVersion is designed to record and label the controllerRevision of sidecarSet.
	// This field will be passed from SidecarSet to its corresponding ControllerRevision, users can use
	// this field to selector the ControllerRevision they want.
	// For example, users can update the customVersion from "version-1" to "version-2" when they upgrade the
	// sidecarSet to "version-2", and they write the "version-2" to InjectionStrategy.Revision.CustomVersion
	// when they decided to promote the "version-2", to avoid some risks about gray deployment of SidecarSet.
...
type SidecarSetInjectRevision struct {
	// CustomVersion corresponds to the customVersion field of (History) SidecarSet.
